...
参考：[前端资源汇总(掘金)][allFrontEndResourceUrl]、[你可能需要的前端知识点][frontEndResourceOneUrl]、[中高级葵花宝典][middleAndHighLevelIterviewUrl]、[JavaScript开发者应懂的33个概念][jsEngineerShouldKonw33Concept]、[关于js你需要知道的][aboutJsYouNeedKonw]<br/>
...
在移动端浏览器及某些桌面浏览器中，window对象有`devicePixelRatio`属性，也就是`devicePixelRatio = 物理像素 / 独立像素`。在mac上打印这个值为2，而普通的浏览器是1，这就是所谓的Retina屏。。。另外需要注意，当缩放浏览器窗口后，在终端打印出来的devicePixelRatio会变化
...
***浮动***<br/>
float 属性最初只用于在成块的文本内浮动图像，但是现在它已成为在网页上创建多列布局的最常用工具之一。因此当浮动元素与非浮动元素同级时，会发生排序混乱的情况，这时可以清除浮动来消除这种现象。。。说白了，其实就是通过清除浮动，来将浮动的元素隔离开来
- 如果在页面内加标签元素的话，可以添加clear:both属性;
- 如果是通过给元素添加伪元素after或before，则需要
```css
&::after{
  content: ' ',
  display: block;
...
```
其实设置content为空就是没内容，display为block就是整个横向都清除浮动。这两个属性其实就类似直接在页面内添加个新标签。clear:both就是清除浮动真正起作用的属性，还有clear:right,clear:left;
...
`Blob，Binary Large Object`的缩写，代表二进制类型的大对象。在Web中，Blob类型的对象表示不可变的类似文件对象的原始数据，通俗点说，就是**Blob对象是二进制数据，但它是类似文件对象的二进制数据**，因此可以像操作File对象一样操作Blob对象，实际上，File继承自Blob。
...
```
**务必注意：**类class没有变量提升；类声明和类表达式的主体(其实就是定义类的具体的内容)都执行在严格模式下，比如构造函数，静态方法，原型方法，getter和setter都在严格模式下执行，也就意味着，**this在严格模式下并不会自动被包装成对象**。。。
...
`Cache-control`是http1.1的产物，是控制浏览器和其他中间缓存如何缓存各个响应以及缓存多久。有以下几种取值(多个取值可以逗号分隔)：<br/>
...
4. no-store：直接禁止浏览器以及所有中间缓存存储任何版本的响应
...
例如，假设有一个文件路径为 /root/src/moduleA.js，包含了一个导入var x = require("./moduleB")。Node.js的解析过程为：<br/>
...
参考: [xss攻击示例][xssExampleUrl]、[xss攻击转义][xssAttackDecodeURL]、[常见web攻击][usualWebSecurityUrl]<br/>
...
***导航和打开窗口***<br/>
window.open()方法既可以导航到一个特定的URL，也可以打开一个新的浏览器窗口。有四个参数：

- 要加载的URL
- 窗口目标(可以自定义名，也可以用'_black'则每次都是新页面，还有'_self','_top','_parent')
- 一个设置窗口样式的特性字符串（比如新窗口是否全屏，大小等，逗号分开）
- 新页面是否取代浏览器历史记录中当前加载页面的布尔值(不觉明历)
...
参考：[H5与Native交互之JSBridge技术][h5AndNativeConnectUrl]、[WebViewJavascriptBridge][WebViewJavascriptBridgeUrl]
...
setupWebViewJavascriptBridge( function ( bridge ) {
  ...
  if ( !isPhone ) {
    bridge.init( function ( message, responseCallback ) {
      ...
      alert( 'JS got a message' );
      var data = {
        'Javascript Responds': 'Wee!'
      };
      console.log( 'JS responding with', data );
      alert( 'JS responding with' );
  ...
  // 下面是定义一个jsFunc方法，供原生调用
  bridge.registerHandler( 'finupCredit_bridgeCallJavaScript', function jsFunc () {
    alert( 'this code from js' );
  } );
...
// 原生调用js方法，js需要注册，这里只是将js的方法注入到_data对象里
// 上面
obj.register = function ( name, callbackFunc ) {
  _data[ name ] = callbackFunc;
};

obj.register( 'jsFunc', function () {
  alert( 'ok1' );
} );
...
Native（Objective-C或Swift）调用Javascript方法，只需将js的方法暴露给`window`即可。  
...
[WebViewJavascriptBridgeUrl]: https://github.com/marcuswestin/WebViewJavascriptBridge 'WebViewJavascriptBridge官方'
