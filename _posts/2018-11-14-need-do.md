...
// 兼容跨域重命名失败的场景
export function downloadFile(url, filenamePrefix = '', timeFat = '') {
  ...
  const time = `${y}年${m}月${d}日${hh}时${mm}分${ss}秒`;

  getBlob(url, function (blob) {
    saveAs(blob, `${filenamePrefix}${timeFat || time}.xlsx`);
  });
}
```
...
```js
function copy(target, weakMap) {
    // 引用类型
    if (isTypeofObject(target) && target !== null) {
      var copiedRes = weakMap ? weakMap.get(target) : undefined; // 循环引用，打断递归

      if (copiedRes !== undefined) {
        return copiedRes;
      } // 细分数组与对象
      ...
      if (isArray$1(target)) {
        copiedRes = [];
        weakMap && weakMap.set(target, copiedRes);

        for (var i = 0, len = target.length; i < len; ++i) {
          copiedRes[i] = copy(target[i], weakMap);
        }
      } else {
        copiedRes = {};
        weakMap && weakMap.set(target, copiedRes); // 使用 in 遍历所有可枚举的 key，包含原型链上所有键

        for (var key in target) {
          // 使用 hasOwnProperty 排除原型链上的 key，仅拷贝对象自身的键
          if (hasOwnProperty$1.call(target, key)) {
            copiedRes[key] = copy(target[key], weakMap);
          }
        }
      } // 返回拷贝值
      ...
      return copiedRes;
    } // 基本类型直接按值返回，除了 symbol 类型


    if (_typeof(target) !== 'symbol') {
      return target;
    }
  }
```
...
循环引用是指两个或多个对象相互引用，导致深拷贝时会进入死循环，最终导致堆栈溢出。

为了解决循环引用的问题，可以使用 Map 或 WeakMap 数据结构来记录已经拷贝过的对象，避免重复拷贝。当遇到已经拷贝过的对象时，直接返回对应的拷贝结果即可，避免重复拷贝导致的死循环问题。

Map 和 WeakMap 的区别在于，Map 可以存储任意类型的键和值，**而 WeakMap 只能存储对象作为键，并且弱引用对象，当被引用的对象被垃圾回收时，WeakMap 中对应的键值对也会被自动删除，从而避免内存泄漏问题。由于 WeakMap 的键是弱引用的，所以无法使用 forEach 或者 size 属性来遍历键或者获取键的数量。**

在实际应用中，由于 WeakMap 的特殊性，通常建议使用 WeakMap 来处理循环引用的问题，避免内存泄漏。但是如果需要在深拷贝中使用 Map 时，可以采用一些其他的解决方案，比如在记录循环引用时使用标记对象，或者在处理完循环引用后手动清空 Map 对象，避免内存泄漏。
...
function deepClone(target) {
  ...
  function _deep(target) {
    if (target === null || target === undefined || _getType(target) !== 'Array' && _getType(target) !== 'Object') {
      return target;
    }
    if (tempMap.has(target)) {
      return tempMap.get(target);
    }
...
Array(10).map((item,idex) => idex)
// 这里只是声明一个长度为10的数组空间，但每个空间并没有值
// [empty × 10]
...
面向对象编程是一种基于以下思路的程序设计方法:将关注点置于 对象(Object)本身，**对象的构成要素包含对象的行为及操作B，以此为 基础进行编程**。这种方法使程序易于复用，软件的生产效率因而得以 提升。其中所使用的主要编程技巧有继承、封装、多态三种。继承指的是通过继承已存在的类所拥有的成员而生成新的类。封 装指的是在类所拥有的成员中，隐藏掉那些没有必要展现给该类调用 者的成员。多态指的是针对同一种消息，不同的对象可以进行不同的 操作。


在面向对象编程中，使用了一种称为“类”的要素，通过把若干个 类组装到一起构建一个完整的程序。从这一点来看，可以说类就是程 序的组件(Component)。面向对象编程的关键在于能否灵活地运用类。类是对象的定义，而对象是 类的实例(Instance)。

在使用古老的 C 语言或 BASIC 等语言编程时(它们不是面向对象 的编程语言，即不是用于表达面向对象编程思想的语言)，用“函数” 表示指令，用“变量”表示数据。对于 C 语言或是 BASIC 的程序员而 言，程序就是函数和数据的集合。

Java 和 .NET 其实是位于操作系统(Windows 或 Linux 等)之上，旨在通过隐藏操作系统的复杂性从而提升开发效率的 程序集，**这样的程序集也被称作“框架”(Framework)。框架由两部分构成，一部分是负责安全执行程序的“执行引擎”，另一部分是作为程 序组件集合的“类库”**。

![java框架图示](/jsArt/assets/images/js-theory/java-frame.png)

无论是使用 Java 还是 .NET，都需要依赖类库进行面向对象编程。 在 Java 中，使用的是与框架同名的 Java 语言。而在 .NET 中，使用的 是 .NET 框 架 支 持 的 C#、Visual Basic.NET、Visual C++、NetCOBOL 等语言进行开发。
...
var event = {
  ...
  remove: function(){
    ...
    if(!fns) return false
    // 如果没有传具体的回调函数，表���需要取消key对应消息的所有订阅
    if(!fn){
      fns.length = 0
    }else{
...
- ISO: international Origanization for Standards (国际标准化组织)
- OSI: open Systems Interconnection（开发式通信系统互联参考模型）
- IETF: Internet Engineeering Task Force（非国家或国际机构等公共机构所制定的标准，但属于业界公认的标准），制定了TCP/IP
- TCP/IP：最初的网络鼻祖ARPANET(阿帕网)，是大学的项目，只是军事正好也需要，因此助推了网络的逐渐成型，而tcp/ip的成型源于UNIX实现了初版的TCP/IP协议，后来才慢慢被阿帕网采用，并推广，最后成为了互联网。
- ISP: 等到网络出来后，就诞生了提供互联网接入服务的公司（internet service provider）
- 以太网：在爱因斯坦提出量子力学之前，人们普遍认为宇宙内充满以太，并以波的形式传送光。而以太网规范简单，易于网卡及驱动程序的实现，且以太网网卡比较便宜，因此得以普及。以太网是一种通信方式，还是需要电缆进行传输信号的。计算机内部采用二进制 1K = 1024，1M = 1024K；而在以太网中以时钟频率决定传输速度，二者并不同，1k = 1000，1M = 1000K。以太网有自己的数据格式（阮一峰的帧）
- 无线通信，通过电磁波，红外线、激光等传输信号；Wi-Fi(wireless fidelity)高质量的无限lan。蓝牙也是一种
...
子网掩码的作用是标识出在 32 比特的 IP 地 址中，从哪一位到哪一位是网络地址，从哪一位到哪一位是主机地址。网掩码中，值为 1 的那些位对应着 IP 地址中的网络地址，后面 值为 0 的那些位则对应着主机地址。

虽然在这个对话框中可以手动设置 IP 地址和子网掩码，但是大多 数情况下选择的还是“自动获得 IP 地址”这个选项。这个选项使得计 算机在启动时会去从 DHCP(Dynamic Host Configuration Protocol(动态主机设 置协议) 服务器获取 IP 地址和子网掩码，并自动地 配置它们。

TCP/IP 这个词表示在网络上 同时使用了 TCP 和 IP 这两种协议。正如前面所讲解的那样，IP 协议 用于指定数据发送目的地的 IP 地址以及通过路由器转发数据。而 TCP 协议则用于通过数据发送者和接收者相互回应对方发来的确认信 号，可靠地传输数据。通常把像这样的数据传送方式称作“握手”,(Handshake)(如图 9.13 所示)。TCP 协议中还规定，发送者要先把原 始的大数据分割成以“包”(Packet)为单位的数据单元，然后再发送， 而接收者要把收到的包拼装在一起还原出原始数据。

例如，诸位敲打键盘输入的电子邮件 正文等数据，并不是原封不动地发送出去的，而是先通过实现了 TCP 协 议的程序附加上遵守 TCP 约束所需的信息，然后再通过实现了 IP 协议 的程序，进一步附加上遵守 IP 约束所需的信息。实际上计算机发送的 是以包为单位的、附加了各种各样信息的数据

硬件上发送数据的是网卡。在网卡之上是设备驱动程序(用于控制 网卡这类硬件的程序)，设备驱动程序之上是实现了 IP 协议的程序，IP 程序之上则是实现了 TCP 协议的程序，而再往上才是应用程序，比如 Web 或电子邮件。这
...
路由器虽然看起来就是个小盒子，可实际上是一台神奇的计算机。 分布在世界各地的 LAN 中的路由器相互交换着信息，互联网正是由于 这种信息的交换才得以联通。这种信息被称作“路由表”，用来记录应 该把数据转发到哪里

- 通常把在一栋建筑物内或是一间办公室里的那种小规 模网络称作 LAN。与此相对，把互联网那样的大规模 网络称作 WAN(Wide Area Network，广域网)。
- “集线器”(Hub)是负责把各台计算机的网线相互连接在一起集线设备
- “路由器”(Router)是负责把公司内的网络和 互联网连接起来的设备。
- 可以查看路由表（route），还可以查看dns(nslookup)寻址表

通常把在一栋建筑物内或是一间办公室里的那种小规 模网络称作 LAN。与此相对，把互联网那样的大规模 网络称作 WAN(Wide Area Network，广域网)。

VLAN（Virtual Local Area Network），在进行网络管理的时候，时常遇到分散网络负载、变换部署网络设备的位置等情况，此时就得修改网络的拓扑结构以及硬件线路的改造，然而使用VLAN就不需要，只需要网络的结构即可。
...
跨域是浏览器的安全策略，两个服务之间通过httpClient 通信是不会有跨越问题的，如果本机使用npx serve 启动两个服务，互相访问。。。是没有跨域问题的
...
// 其实查看以下，204时，是 OPTIONS 请求，因此没有响应体。
```
...
304 // Not Modified ，一般���针对get或put请求？
```
...
参考：https://juejin.im/post/6844903801778864136
...
`Cache-Control: max-age= 7200, public, immutable`这三个值可以同时使用，以提供更详细的缓存控制策略。在给定的示例中，max-age=7200 指定了缓存的最长时间为 2 小时，public 允许公共缓存服务器缓存该资源，immutable 表示资源是不可变的，可以放心地进行缓存。这样可以在一段时间内，让客户端和中间缓存服务器直接使用缓存的资源，而无需频繁地向服务器请求相同的资源，提升了性能和效率。
...
并非所有服务器都使用文件内容的哈希值作为ETag的生成方式。有些服务器可能使用其他算法或策略来生成ETag，例如基于文件的元数据、修改时间戳等。在这种情况下，ETag值的变化并不能直接说明文件内容已经更改。
...
在因特网上传送URL，只能采用ASCII字符集，也就是说URL只能使用英文字母、阿拉伯数字和某些标点符号，不能使用其他文字和符号，即只有字母和数字[0-9a-zA-Z]、一些特殊符号$-_.+!*'()[不包括双引号]、以及某些保留字（空格转换为+），才可以不经过编码直接用于URL，这意味着 如果URL中有汉字，就必须编码后使用。 但是麻烦的是 标准的国际组织并没有规定具体的编码方法，而是交给应用程序（浏览器）自己决定。 这导致"URL编码"成为了一个混乱的领域。
...
// 解码被编码多次的url
let str = "https%3A%2F%2Fwww.baidu.com%2F%3Fa%3D%25E4%25B8%25AD"
const isEncode = str => str.includes('%25');
const getDecodeUrl = str => {
    while(isEncode(str)) {
...
[视频编码](https://juejin.im/post/5dd359f4e51d453af47cea29)


### DRM，即数字版权管理（Digital Rights Management）
...
而客户端呢就需要相应的获得解密的秘钥对视频内容解密,值得注意的是DRM里面秘钥一般被称license而不是key。
...
.npmrc 是 NPM（Node Package Manager）的配置文件，用于配置 NPM 的行为和特性。它可以用来配置多个 NPM 相关的选项，例如：

- 指定默认的安装目录：可以通过配置 prefix 选项来指定默认的安装目录，让你在全局安装 NPM 包时不必每次都指定安装目录。
- 指定安装源：可以通过配置 registry 选项来指定 NPM 的安装源，加速包的下载和安装。
- 认证信息：可以配置 username 和 password 选项来保存 NPM 的认证信息，避免每次都需要手动输入用户名和密码。
- 镜像源：可以通过配置 disturl 选项来指定 NPM 的镜像源，加快包的下载速度。
- 包的版本控制：可以配置 save-prefix 选项来指定包的版本前缀，让你在安装包时使用指定的版本前缀，避免出现版本兼容性问题。

私有库的话，需要配置register ，比如`npm config set @mfs:registry http://xxx.net/`，当然也可以直接配置`~/.npmrc`，如果在项目里配置了`.npmrc`文件也可以，比如：

```bash
# .npmrc
@mfs:registry=http://registry.npm.missfresh.net
@mfb:registry=https://registry.npm.taobao.org
```

这个配置选项中，@mfb 是一个 NPM 命名空间（scope），表示该配置项只对属于 @mfb 命名空间下的包有效。命名空间是一种用于将相关的包分组的机制，有助于包的管理和组织。在 NPM 中，命名空间以 @ 符号开头，例如 @babel、@angular 等。

https://registry.npm.taobao.org 则是 NPM 的安装源地址，表示该配置项指定使用淘宝 NPM 镜像源来下载和安装属于 @mfb 命名空间的包。


本地项目npm 下载资源时都从哪些文件里查找registry配置？

当我们在本地使用 NPM 下载资源时，NPM 会首先查找当前目录下的 .npmrc 文件，如果不存在，则会依次查找 $HOME/.npmrc、$HOME/.config/npm/npmrc 和 $PREFIX/etc/npmrc 等系统级的 .npmrc 文件，以获取当前 NPM 的配置信息，其中 $HOME 和 $PREFIX 分别表示系统的家目录和 NPM 的安装路径。

在这些 .npmrc 文件中，可以通过设置 registry 选项来指定 NPM 的包管理源地址。例如，设置 registry 选项为 https://registry.npm.taobao.org 即表示使用淘宝 NPM 镜像源来下载资源，可以加快下载速度。


.npmrc，可以理解成npm running cnfiguration, 即npm运行时配置文件。我们知道，npm最大的作用就是帮助开发者安装需要的依赖包，
...
authToken 是 npm 用户登陆仓库时，由npm仓库生成返回给客户端，记录到客户端的 .npmrc 中

如何获取authToken
1. 设置 npm 仓库：npm set registry <xxx>
2. 交互式登录 npm 仓库：npm login ，输入用户名和密码，完成登录
   1. 如果之前没有添加过用户，可以 npm adduser，添加账号（公司的用户名一般是邮箱名和邮箱密码）
3. 打开.npmrc文件，即可看到对应的authToken 

.npmrc文件中的_authToken（即身份验证令牌）没有过期时间，这意味着一旦生成了身份验证令牌，它将一直有效，除非您手动撤销或更改它。

npm发包其实就是将自己的仓库标准化并公开给所有人，然后用户通过npm search就可以找到包(如果没有发包，则需要找到对应的仓库去克隆)，这里的npm search在使用淘宝镜像的情况下不太好使。。。

#### .npmrc 的秘钥配置

//npm.xxx.com/:_authToken=xxx 是一个 .npmrc 文件中的配置选项，用于保存 NPM 私有包的认证信息，以便于在安装和发布私有包时进行身份验证。

在这个配置选项中，//npm.xxx.com/ 是 NPM 私有包的访问地址，:_authToken 则是 NPM 的认证机制。在使用 NPM 私有包时，用户需要先登录到该包的私有库，并获得一个认证 Token（即 _authToken），然后将该 Token 保存到 .npmrc 文件中(一般在家目录，登录后这个文件会更新)，以便于 NPM 在安装和发布私有包时进行身份验证。

保存 _authToken 的方法通常是使用 npm login 命令登录到私有包的库，登录成功后 NPM 会自动将认证信息保存到 .npmrc 文件中，例如：

```bash
npm login --registry=https://npm.xxx.com
Username: your-username
Password: your-password
Email: (this IS public) your-email@example.com
# Logged in as your-username on https://npm.xxx.com/.
```

登录成功后，.npmrc 文件中就会自动添加一行 //npm.xxx.com/:_authToken=xxx，其中 xxx 表示登录成功后获得的认证 Token。这样，在安装或发布私有包时，NPM 就会使用保存在 .npmrc 文件中的认证 Token 来进行身份验证。
...
CSS in JS是指在JavaScript中编写CSS样式，使用一些特定的库或框架，如styled-components、emotion等，可以直接在组件内部定义CSS样式，将CSS样式和组件逻辑紧密结合起来。这种方式可以避免全局命名冲突和作用域问题，使得代码更加模块化和可维护。其实就是用css 写js组件

CSS Modules是指使用webpack等打包工具，将CSS文件中的类名进行局部作用域处理，生成唯一的类名，并在组件中通过import的方式引入。这种方式同样可以避免全局命名冲突和作用域问题，但是需要在组件中显式引入CSS文件，不够灵活。
...
虚拟主机部署 Web 服务和 Docker 容器部署 Web 服务的主要区别在于：

- 隔离性不同：虚拟主机通过隔离多个 Web 应用程序的文件系统、网络和系统资源，实现了基本的隔离；而 Docker 则提供了更强的隔离性，容器中的应用程序可以看到自己的独立文件系统、网络和系统资源，并与其他容器和主机隔离开来。
- 环境配置不同：在虚拟主机中，每个应用程序都需要自己的环境配置，如操作系统、Web 服务器、数据库等，这些需要手动安装和配置；而在 Docker 中，应用程序与其依赖的环境被打包成一个镜像，可以通过简单的命令部署和运行，极大地简化了部署和运维工作。
- 资源利用率不同：在虚拟主机中，每个应用程序都需要占用一定的系统资源，如内存、CPU 和磁盘空间，这可能导致资源浪费和性能下降；而 Docker 则可以更好地利用系统资源，多个容器可以运行在同一台主机上，共享主机的资源，提高了系统的利用率。
- 可移植性不同：Docker 容器可以在不同的环境中运行，如开发、测试和生产环境，而无需担心依赖问题；而在虚拟主机中，由于每个应用程序的环境配置不同，很难在不同环境中进行移植。

需要注意的是，虚拟主机和 Docker 并不是互相排斥的技术，它们可以结合使用，提供更好的部署和运维体验。

制作docker镜像？？

1、创建一个文件夹，并在文件夹中创建一个名为 app.js 的 JavaScript 文件，内容如下：
...
2、在同一文件夹中创建一个名为 Dockerfile 的文件，内容如下：
...
3、在终端中进入该文件夹，并使用以下命令构建 Docker 镜像：
...
5、访问 http://localhost:3000，应该可以看到 "Hello, Docker!" 的响应。
...
[如何制作容器镜像？](https://support.huaweicloud.com/swr_faq/swr_faq_0012.html)
镜像制作的两种方式：
- 方法 1：docker commit # 保存 container 的当前状态到 image 后，然后生成对应的 image
- 方法 2：docker build # 使用 Dockerfile 文件自动化制作 image
  
方式一主要是就是快照的方式，适用于偶尔制作镜像，后续不怎么变化的场景，比如在基础镜像上（比如Ubuntu），先登录镜像系统并安装容器引擎软件，然后整体制作快照。大概步骤如下：

1. 找一台主机，安装容器引擎软件。
2. 启动一个空白的基础容器，并进入容器。
  例如：启动一个CentOS的容器： docker run -it centos
3. 执行安装任务。
```bash
yum install XXX
git clone https://xxx
...
方式二：如果后续镜像经常变更（例如某个软件更新版本），则需要采用方法二制作镜像。若仍采用方法一制作镜像，则每次变更都需要重新执行方法一的命令，操作过程比较繁琐，所以建议使用自动化制作镜像的方法。

其实就是将方法一制作镜像的方法，用文件方式写出来（文件名为Dockerfile）。然后执行：docker build -t test/image:tag.命令（命令中“.”表示Dockerfile文件的路径），自动完成镜像制作。

其实就是方式一，需要登录一台物理机，然后安装一个安装容器引擎软件，登录容器，然后安装软件，最后快照。。。后者的话其实就是将前面安装容器开始的步骤写到dockerfile文件里，因此更加方便，直接在物理机上即可操作。


1. FROM指定基础镜像，其实也就是在什么镜像的基础上扩展，比如 ubuntu、debian等等。当然如果不依赖任何镜像，还可以`FROM scratch`;<br/>
2. RUN执行命令，有以下两种格式：
  - shell 格式：RUN <命令>
  - exec 格式：RUN ["可执行文件", "参数1", "参数2"]，这更像是函数调用中的格式。
  - 一条RUN指令执行完毕后，会在当前镜像上创建一个新的镜像层，接下来的指令会在新的镜像上继续执行。
3. ADD，向新镜像中添加文件，这个文件可以是一个主机文件，也可以是一个网络文件，也可以是一个文件夹。其实就是将一些源文件放进去
4. VOLUME：
...
Dockerfile 中每一个指令都会建立一层，RUN 也不例外。因此多次执行命令就会创建一个很多层的镜像，而有时很多运行时不需要的东西，都被装进了镜像里，比如编译环境、更新的软件包等等。结果就是产生非常臃肿、非常多层的镜像，不仅仅增加了构建部署的时间，也很容易出错。 
...
[docker入门到实践(佳)](https://yeasy.gitbook.io/docker_practice/image/build)
...
**虚拟内存:**是计算机系统内存管理的一种技术。它使得应用程序认为它拥有连续的可用的内存（一个连续完整的地址空间），而实际上，它通常是被分隔成多个物理内存碎片，还有部分暂时存储在外部磁盘存储器上，在需要时进行数据交换。目前，大多数操作系统都使用了虚拟内存，如Windows家族的“虚拟内存”；Linux的“交换空间”等。
