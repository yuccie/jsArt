...
参考：[前端资源汇总(掘金)][allFrontEndResourceUrl]、[你可能需要的前端知识点][frontEndResourceOneUrl]、[中高级葵花宝典][middleAndHighLevelIterviewUrl]、[JavaScript开发者应懂的33个概念][jsEngineerShouldKonw33Concept]、[关于js你需要知道的][aboutJsYouNeedKonw]  
...
**其实**问题就是1px表示的设备像素多了，因此想法让1px表示的减少即可，比如缩放，比如根据dpr动态设置
...
其实说到底，只要margin-top和margin-bottom之间没有**一些东西**隔开，就会发生合并。。。而这里的**一些东西**可以是：边框、内边距、行内内容、height、min-height 等。  
...
1. window.sessionStorage是会话期间(关闭浏览器会话结束,刷新后也有效)
...
// 前者主要兼容pc端，后者主要针对移动端（新document.scrollingElement自动兼容）
...
// 这种深拷贝效果更佳
function goodDeepCopy(target) {
  // 用WeakMap效率更高
  let copyed_objs = new WeakMap;
  function _deepCopy(target) {
    if (typeof target !== "object" || !target) {
      return target;
    }
    if (copyed_objs.has(target)) {
      return copyed_objs.get(target);
    }
    let obj = {};
    if (Array.isArray(target)) {
      obj = []; //处理target是数组的情况
    }
    copyed_objs.set(target, obj);
    Object.keys(target).forEach(key => {
      if (obj[key]) {
        return;
      }
...
标识：`Expires`是http1.0的产物，值为服务器返回该请求结果缓存的到期时间(绝对时间，若身处不同时区则不准确)；  
`Cache-control`是http1.1的产物，是控制浏览器和其他中间缓存如何缓存各个响应以及缓存多久。有以下几种取值(多个取值可以逗号分隔)：  
...
普通xss 攻击，通过html 转意就可以很好地解决，但是富文本编辑器，本身就是允许输入html 标签的，不能转义，需要引入第三方防止xss的包来处理，对文章内html 进行处，可以自定义过滤规则，参考：[根据白名单过滤HTML(防止XSS攻击)](https://jsxss.com/zh/index.html)
