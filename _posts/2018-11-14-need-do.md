...
// abbba是真个匹配的结果，第一个括号匹配的就是bbb
=> ["abbba", "bbb", index: 1, input: "_abbba_aba_", groups: undefined]
...
function getQuery() {
  ...
  // 匹配 ie=utf-8&f=8&rsv_bp=1&tn=baidu&wd 类似这样的值
  let reg = /([^&=]+)=?([^&]*)/g;
...
var m = 'abc'.match(/b(?=c)/);
m // ["b"]，其实这里match的前两项分别是整个匹配结果，第二项是匹配组项，这里只是不展示匹配组项而已
var m = 'abc'.match(/b(c)/);
m // ["bc", "b"]
