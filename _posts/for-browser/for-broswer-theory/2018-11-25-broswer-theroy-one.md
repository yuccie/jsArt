...
函数调用栈溢出是指在程序运行时，函数调用栈的空间超过了系统限制，导致程序崩溃或出现其他错误。函数调用栈是程序中用于存储函数调用信息的一种数据结构，每当一个函数被调用时，就会在栈上分配一段内存用于保存该函数的参数、局部变量和返回地址等信息。当函数返回时，这些信息就会从栈上弹出，恢复之前的函数调用。

函数调用栈溢出的原因通常有以下几个：

- 递归调用：递归函数是指在函数内部调用自身的函数。如果递归的深度过大，就会导致函数调用栈的空间不足，从而发生栈溢出。
- 大量的局部变量：如果函数中定义了大量的局部变量，也会占用大量的函数调用栈空间，从而导致栈溢出。
- 函数嵌套调用：如果函数中嵌套调用了大量的函数，也会占用大量的函数调用栈空间，从而导致栈溢出。
- 参数传递错误：如果函数的参数传递错误，如参数类型不匹配、传递过多的参数等，也会导致栈溢出。
- 系统限制：操作系统对函数调用栈的大小通常有一定的限制，如果超过了这个限制，也会导致栈溢出。
...
因为字符串的数组行为不标准，所以应该避免使用它。因此对字符串类型的变量，使用下标的形式来取得其中的某个字符本来就是不规范的，最正确的做法是使用 charAt(index) 方法。另：
...
"Profile" 模式指的是性能分析模式，用于对应用程序进行性能分析，以便识别和优化性能瓶颈。在该模式下，开发人员可以监控应用程序的 CPU、内存、网络等性能指标，并分析应用程序在执行时的行为和资源消耗情况，从而找出性能瓶颈并优化应用程序的性能。常见的工具包括 Chrome DevTools 中的 Performance 面板、Node.js 中的 CPU Profiling 和 Heap Profiling 等。
...
在 JavaScript 中，内部函数返回给上层的对象和普通对象一样，都是通过引用来进行管理和访问的。当一个函数返回一个对象时，该对象会被存储在堆内存中，并在该函数执行完成后被返回给调用方，调用方可以通过引用来访问该对象。

当没有任何引用指向一个对象时，该对象就变成了垃圾数据。
...
而 JavaScript 引擎在解析 JavaScript 之前，是不知道 JavaScript 是否操作 CSSOM 的，**所以渲染引擎在遇到 JavaScript 脚本时，不管该脚本是否操做了 CSSOM，都会先执行 CSS 文件下载，解析操作，再执行 JavaScript 脚本**。
...
dom解析和css解析是两个并行的线程，所以**css加载不会阻塞dom的解析**
...
如果第三方服务器不支持 CORS，那么客户端请求同样会作废，因为客户端会验证响应中是否包含选择统一的首部。作为一个特例，CORS 还允许服务器返回一个通配值 (Access-Control-Allow-Origin: *)，表示它允许 来自任何源的请求。不过，在启用这个选项前，请大家务必三思!
...
- 在杭州看房，买房方便。在北京买房落户几乎无望，本人还是倾向于努力并享受当下，而不是以后。。。
- 在杭州落户需要半年以上社保。
- 另外就是我家那位，职业该认真考虑哪个方向了，不能再没有规划了。
- 还有就是年纪也确实不小了，得成家立业了。
- 去杭州就可以考虑买车，无人机等也能飞了，就可以开始下阶段的自驾游了。
- 我这边不安定，丈母娘那边不放心。
- 可以专心在一个地方努力了，毕竟终要择一城终老。
- 家人大多都在杭州，后续什么都发方便。
- 和女朋友，能不能走到一起，也是一次终极考验。
#### 继续在北京

- 尝试内调，如果不行，则需要重新找工作。
- 工资没有多少优势，但房租比杭州贵上2/3。
- 北方风景确实不如南方，南方天气宜居，前期可能不适应。
- 冬季可以滑雪。
#### HTTPS：让数据传输更安全
...
堆是一种数据结构，用于存储和管理动态分配的内存。在计算机内存中，堆是一个连续的区域，用于存储动态分配的内存块。堆与栈不同，栈是存储函数和局部变量的内存区域，而堆是用于存储动态分配的内存块的内存区域。

堆中的每个内存块都有一个大小，可以根据需要分配或释放。当需要分配内存时，堆会在内存中寻找足够大的连续空间，并返回一个指向该空间的指针。当需要释放内存时，堆会将该内存块标记为可用，并将其返回给堆，以便再次使用。

堆还可以实现一些高级数据结构，例如二叉堆和斐波那契堆，用于优先队列和最小生成树算法等。这些数据结构使用堆来实现高效的插入、删除和查找操作，以及维护优先级顺序。


**在 JavaScript 中，一个数据存储在堆内存中的全过程包括以下几个步骤：**

申请内存空间：在 JavaScript 中，可以使用 new 关键字或者字面量的方式来创建一个对象，当创建一个对象时，JavaScript 引擎会根据对象的大小在堆内存中申请一段连续的内存空间。

分配地址：在堆内存中分配一段连续的内存空间后，JavaScript 引擎会为该对象分配一个地址，并返回该地址的引用。

存储数据：JavaScript 引擎会将对象的属性和方法存储在堆内存中的对应地址处，对象的数据和方法都是通过引用来访问的。

访问数据：访问堆内存中的数据时，需要通过引用来定位对象的地址，然后再根据属性名或者方法名来访问对象中的数据。

回收内存：当一个对象不再被引用时，JavaScript 引擎会将该对象标记为垃圾数据，然后由垃圾回收器回收其占用的内存空间。具体的回收方式和时间取决于具体的垃圾回收算法和实现。

需要注意的是，堆内存中存储的数据是动态分配的，程序员需要手动管理堆内存中的对象，包括创建、使用和释放内存。如果对象没有被垃圾回收器回收，可能会导致内存泄漏和性能问题。因此，在编写 JavaScript 代码时，需要注意对象的内存管理问题，避免出现内存泄漏和性能问题。
...
堆内存和栈内存都是计算机内存中的一部分，它们之间有以下几个区别：

分配方式：栈内存的分配方式是静态的，编译器会在编译时确定变量的内存大小和位置，而堆内存的分配方式是动态的，需要在运行时通过代码进行分配。

空间大小：栈内存空间通常比较小，因为它是预先分配的并且大小是固定的，而堆内存空间通常比较大，因为它可以在运行时动态分配。

内存管理：栈内存的管理是自动的，由编译器自动分配和释放，而堆内存的管理是手动的，需要程序员显式地分配和释放。

访问速度：由于栈内存是预先分配的，所以它的访问速度比堆内存更快。而堆内存的访问速度较慢，因为需要通过指针进行访问。

存储内容：栈内存通常用于存储局部变量和函数调用的上下文信息等，而堆内存通常用于存储动态分配的对象，如数组和对象等。

需要注意的是，堆内存和栈内存并不是完全独立的，它们之间可以相互调用和使用。在函数调用时，函数的参数和返回值通常是通过栈内存传递的，而函数内部分配的局部变量也通常是在栈内存中分配的。而当需要动态分配内存时，通常使用堆内存来分配。

**数组在内存中是放在堆内存还是栈内存？**

数组在内存中的存储位置，取决于数组是在堆内存还是栈内存中分配的。

- 在静态内存分配的情况下，数组通常被分配在栈内存中。当我们在函数内部定义一个数组时，它会被分配在栈内存中。这些数组的大小是固定的，在编译时就已经确定，所以它们可以被静态地分配在栈内存中。当函数执行完毕后，栈内存中的数组也会被自动释放。

- 在动态内存分配的情况下，数组通常被分配在堆内存中。当我们使用动态内存分配函数（如malloc()或new）分配数组时，它们会被分配在堆内存中。这些数组的大小是在运行时确定的，所以它们需要动态地分配在堆内存中。在使用完堆内存中的数组后，我们需要手动释放内存，以免出现内存泄漏的问题。

需要注意的是，数组中的元素存储在数组内存空间中，而数组本身只是一个指向这些元素的指针。因此，无论数组是在堆内存还是栈内存中分配的，元素的存储位置都是相同的。

在大多数情况下，数组在内存中的存储空间是连续的。这是由于数组在内存中的存储顺序是按照数组下标的顺序进行排列的，而内存中的地址也是连续的。

但是，数组在内存中的存储空间不一定总是连续的。例如，当数组被分配在堆内存中时，它的存储空间可能不是连续的，因为堆内存中的空间是动态分配的，可能存在空洞或碎片。此外，当数组中的元素是引用类型时，它们的存储空间也可能不是连续的，因为它们在内存中的存储位置是由它们的引用决定的。
...
例如，主机名与ip地址的对应信息叫**A记录**，反之，从IP地址检索主机名称的信息叫**PTR**，上层或下层域名服务器IP地址映射叫做**NS记录**。而CNAME记录就是把域名解析到另外一个域名。
...
UDP中的user，不单单指互联网的使用者，曾经也是为了表示哪些编写程序的程序员，也就是说udp是按照程序员的编程思想在传送数据。
...
- 强制缓存生效期间，前端请求头是不会发送协商缓存的请求头标识的
- catch-control也可以在请求头里添加。
- 当强制失效，才会携带协商缓存的标识去服务端。
