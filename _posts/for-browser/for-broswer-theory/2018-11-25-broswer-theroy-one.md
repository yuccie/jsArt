...
IPC：每个进程各自有不同的用户地址空间，任何一个进程的全局变量在另一个进程中都看不到，所以进程之间要交换数据必须通过内核，在内核中开辟一块缓冲区，进程1把数据从用户空间拷到内核缓冲区，进程2再从内核缓冲区把数据读走，内核提供的这种机制称为进程间通信（IPC，InterProcess Communication）

**安全沙箱**：可以把沙箱看成是操作系统给进程上了一把锁，沙箱里面的程序可以运行，但是不能在你的硬盘上写入任何数据，也不能在敏感位置读取任何数据

PAC，全名为 proxy auto-config。一个自动代理配置脚本，包含了很多使用 JavaScript 编写的规则，它能够决定网络流量走默认通道还是代理服务器通道，控制的流量类型包括：HTTP、HTTPS 和 FTP。
参考：https://www.barretlee.com/blog/2016/08/25/pac-file/

iframe没有单独标签，是嵌入在其他页面里面的，比如一个页面嵌入了三个不同域名的iframe，那么这个页面就会拥有四个渲染进程
...
以上四个进程是必须的，但是通常情况下，会有更多进程，如**扩展进程，代理进程，iframe进程**，还会一些如压缩，视频，音频等功能进程，这个根据每个人的电脑环境，页面环境不同而不同！

另外Chrome有一些辅助进程在任务管理器并没有显示出来的。比如预渲进程，会有一个额外的渲染进程，提前开启，等有渲染进程需求的时候，就直接使用改进程，这样就省去了创建进程的时间了！

单核和多核，线程与进程：
- 单核就是CPU集成了一个运算核心，在工作期间只能执行某一个程序，处理多个程序时，只能分时处理。参考：https://cloud.tencent.com/developer/article/1456363
...
curl -I http://www.baidu.com 返回的信息是200，但是浏览器直接输入 http 协议的时候百度会跳转到 https，这个是为什么啊？那百度是怎么做的重定向呢？
...
**注意：**websocket名字取的比较有迷惑性，其实和socket完全不一样，你可以把websocket看出是http的改造版本，增加了服务器向客户端主动发送消息的能力，就相当于java和javascript的区别
...
那是值得纪念的日子，就在发版前几天，又增加了一些关键的功能......我知 道 MSXML 库要随 IE 一起发布，我跟 XML 团队关系还不错，觉得他们或 许能帮个忙。我找到当时的团队领导 Jean Paoli，很快就说服他同意把它作 为 MSXML 库的一部分发布。它其实主要跟 HTTP 相关，跟 XML 基本上没 什么关系。仅仅是为了让它能早点发布，我才硬给它起了一个带 XML 的名字。 ——Alex Hopmann The story of XMLHTTP

Mozilla 按照微软的实现也实现了自己的 XHR，并将其命名为 XMLHttpRequest。 Safari、Opera 和其他浏览器也紧随其后，于是 XHR 成为了所有主流浏览器中的事 实标准。W3C 针对 XHR 的官方工作草案发布于 2006 年，而这已经是 XHR 得到广 泛应用以后的事了!XHR 是一个浏览器层面的 API


XMLHttpRequest(XHR)是浏览器层面的 API，可以让开发人员通过 JavaScript实现数据传输。XHR 是在 Internet Explorer 5 中首次亮相的，后来成为 AJAX (Asynchronous JavaScript and XML)革命的核心技术，是今天几乎所有 Web 应用
必不可少的基本构件。XMLHTTP 改变了一切。它让 DHTML 中的 D 变得名副其实。
...
每个 TCP 分组都会带着一个唯一的序列号被发出，而所有分组必须按顺序传送到接收端。如果中途有一个分组没能到达接收 端，那么后续分组必须保存在接收端的 TCP 缓冲区，等待丢失的分组重发并到达接 收端。这一切都发生在 TCP 层，应用程序对 TCP 重发和缓冲区中排队的分组一无所 知，必须等待分组全部到达才能访问数据。在此之前，应用程序只能在通过套接字 读数据时感觉到延迟交付。这种效应称为 TCP 的队首(HOL，Head of Line)阻塞。
...
请求发出后，浏览器自动追加受保 护的 Origin HTTP 首部，包含着发出请求的来源。相应地，远程服务器可以检查 Origin 首部，决定是否接受该请求，如果接受就返回 Access-Control-Allow-Origin 响应首部:

如果第三方服务器不支持 CORS，那么客户端请求同样会作废，因为客户
端会验证响应中是否包含选择同意的首部。作为一个特例，CORS 还允许
服务器返回一个通配值 (Access-Control-Allow-Origin: *)，表示它允许 来自任何源的请求。不过，在启用这个选项前，请大家务必三思!
这就是全部了吧?准确地讲，不是。因为 CORS 还会提前采取一系列安全措施，以 确保服务器支持 CORS:
• CORS 请求会省略 cookie 和 HTTP 认证等用户凭据;
• 客户端被限制只能发送“简单的跨源请求”，包括只能使用特定的方法(GET、
POST 和 HEAD)，以及只能访问可以通过 XHR 发送并读取的 HTTP 首部。
要启用 cookie 和 HTTP 认证，客户端必须在发送请求时通过 XHR 对象发送额外 的属性(withCredentials)，而服务器也必须以适当的首部(Access-Control-Allow- Credentials)响应，表示它允许应用发送用户的隐私数据。类似地，如果客户端需要 写或者读自定义的 HTTP 首部，或者想要使用“不简单的方法”发送请求，那么它 必须首先要获得第三方服务器的许可，即向第三方服务器发送一个预备(preflight) 请求

这里说明了为何要发送预检请求，以及跨域响应是到达浏览器的，只是被浏览器放弃了；

XHR 既可以传输文本数据，也可以传输二进制数据。事实上，浏览器可以自动为各 种原生数据类型提供编码和解码服务，因此应用在直接将这些数据传给 XHR 时就 已经编码 / 解码好了，反之亦然。
...
垃圾数据产生，
```js
// 函数执行完了，执行栈已经清空，但是name变量已经挂载在window上了。
!function (){
  name = '这是临时垃圾变量';
}()
```
...
3. 做内存整理。一般来说，频繁回收对象后，内存中就会存在大量不连续空间，我们把这些不连续的内存空间称为内存碎片。当内存中出现了大量的内存碎片之后，如果需要分配较大的连续内存时，就有可能出现内存不足的情况，所以最后一步需要整理这些内存碎片。但这步其实是可选的，因为有的垃圾回收器不会产生内存碎片，比如接下来我们要介绍的副垃圾回收器。
...
网络为什么要分层？因为，是个复杂的程序都要分层。复杂的程序都要分层，这是程序设计的要求。比如，复杂的电商还会分数据库层、缓存层、Compose 层、Controller 层和接入层，每一层专注做本层的事情。
...
- 在 Linux 上是 ifconfig。还可以 ip addr

**这个命令显示了这台机器上所有的网卡**。大部分的网卡都会有一个 IP 地址，当然，这不是必须的。

32位的ip地址，根据网络号的长度又分为A/B/C/D/E几种，网络号越长，真正能用的ip地址就越少（A类地址最多也只能有128个。。。）。因而出现了自定义网络号长度的方式：无类型域间选路，简称 CIDR（classless inter-domain routing）。这种方式打破了原来设计的几类地址的做法，将 32 位的 IP 地址一分为二，前面是网络号，后面是主机号。从哪里分呢？你如果注意观察的话可以看到，10.100.122.2/24，这个 IP 地址中有一个斜杠，斜杠后面有个数字 24。这种地址表示形式，就是 CIDR。后面 24 的意思是，32 位中，前 24 位是网络号，后 8 位是主机号。

VLSM（variable length subnet mask）可变子网掩码，动态调整子网掩码的长度，从而更加有效的利用ip地址。但绝对的ip地址不够的事实无法改变。

**公有 IP 地址和私有 IP 地址**

私有ip一般是局域网内的，比如一个公司或一个学校里面的ip地址，一般都可以自己控制。而共有ip是对外的，比如阿里的域名是面向全世界的。总结下来：局域网内的一般为私有。

私有地址如果想访问公网，则需要NAT()技术，其实就是二者之间转换了一下，目的也是为环节ip地址少的现状。

共有ip谁管理呢？有ICANN（internet corporation for asigned names and numbers，互联网名称与数字地址分配机构）管理，当然现在的话，一般用户对接的是ISP，其实相当于ISP帮用户申请了共有ip。
...
不需要将十进制转换为二进制 32 位，就能明显看出 192.168.0 是网络号，后面是主机号。而**整个网络里面的第一个地址 192.168.0.1，往往就是你这个私有网络的出口地址**。例如，你家里的电脑连接 Wi-Fi，Wi-Fi 路由器的地址就是 192.168.0.1，而 192.168.0.255 就是广播地址。一旦发送这个地址，整个 192.168.0 网络里面的所有机器都能收到。

但是也不总都是这样的情况。因此，其他情况往往就会很难理解，还容易出错。

我们来看 16.158.165.91/22 这个 CIDR。求一下这个网络的第一个地址、子网掩码和广播地址。

你要是上来就写 16.158.165.1，那就大错特错了。/22 不是 8 的整数倍，不好办，只能先变成二进制来看。

16.158 的部分不会动，它占了前 16 位。中间的 165，变为二进制为‭10100101‬。除了前面的 16 位，还剩 6 位。所以，这 8 位中前 6 位是网络号，16.158.<101001>，而 <01>.91 是机器号。
...
mac地址虽然是独一无二，但无法直接通过mac地址进行通信：因为mac地址目的就是小范围通过广播通信，你没法在北京通过广播来查找上海的某个人的地址。因此就需要配合ip把你先定位到上海的某个地方，然后再广播查找具体的人。其实就是：MAC 地址的通信范围比较小，局限在一个子网里面。

**网络设备的状态标识**

flags=8863<UP,BROADCAST,SMART,RUNNING,SIMPLEX,MULTICAST> mtu 1500，这个叫做 net_device flags，网络设备的状态标识。UP 表示网卡处于启动的状态；BROADCAST 表示这个网卡有广播地址，可以发送广播包；MULTICAST 表示网卡可以发送多播包；LOWER_UP 表示 L1 是启动的，也即网线插着呢。MTU1500 是指什么意思呢？是哪一层的概念呢？最大传输单元 MTU 为 1500，这是以太网的默认值。
...
你以为这样就可以了？这样的话，岂不是让你随便配置了...虽然可以随便配置，但是配置完数据包是发送不出去的。首先发送时需要对方的mac地址，mac地址还需要ip地址通过ARP协议反解析出来。如果不在一个子网里，还需要借助网关。。。因此手动配置ip需要考虑CIDR、子网掩码、广播地址和网关地址等。

#### 动态主机配置协议（DHCP）

Dynamic Host Configuration Protocol，自己配置不但麻烦，当你频繁切换网络时，网络管理员得累死。因此有了这个DHCP协议，网络管理员就轻松多了。他只需要配置一段共享的 IP 地址。每一台新接入的机器都通过 DHCP 协议，来这个共享的 IP 地址里申请，然后自动配置好就可以了。等人走了，或者用完了，还回去，这样其他的机器也能用。

DHCP的工作过程：其实就是新来的主机，通过封装一组预定好的数据格式，然后广播出去，相应的DHCP服务器就会收到请求，然后再按一定的格式处理并通信。如果有多个DHCP服务器，则先到先得。相当于浏览网页，也是需要一个服务器端的。



DHCP一般主要用来分配ip地址，但其实它是一个服务器，还可以做更多服务器可以做的事情，比如一个数据中心里有几百台没有安装系统的机子，可以通过广播方式同时安装几百台机子的系统。这就需要：预启动执行环境（PXE）

首先一个空白的电脑，刚开始只具备一个特别小的小系统，只能干特别小的一件事情。其实就是读取硬盘的 MBR 启动扇区，将 GRUB 启动起来；然后将权力交给 GRUB，GRUB 加载内核、加载作为根文件系统的 initramfs 文件；然后将权力交给内核；最后内核启动，初始化整个操作系统，这就是BIOS。

PXE 协议分为客户端和服务器端，由于还没有操作系统，只能先把客户端放在 BIOS 里面。当计算机启动时，BIOS 把 PXE 客户端调入内存里面，就可以连接到服务端做一些操作了。启动PXE后，DHCP Server 除了分配 IP 地址以外，还会提供其他的一些配置，比如PXE 服务器在哪里，也可以知道如何从 PXE 服务器上下载某个文件，去初始化操作系统。
...
- 如果多台电脑相连可以使用Hub 的东西，也就是集线器，只是简单的将收到的数据复制到其他端口，实现多台电脑相连
...
1. 这个包是发给谁的？谁应该接收？
2. 大家都在发，会不会产生混乱？
3. 有没有谁先发、谁后发的规则？如果发送的时候出现了错误，怎么办？
...
这种组网的方法，对一个宿舍来说没有问题，但是一旦机器数目增多，问题就出现了。因为 Hub 是广播的，不管某个接口是否需要，所有的 Bit 都会被发送出去，然后让主机来判断是不是需要。这种方式路上的车少就没问题，车一多，产生冲突的概率就提高了。而且把不需要的包转发过去，纯属浪费。看来 Hub 这种不管三七二十一都转发的设备是不行了，需要点儿智能的。

因为每个口都只连接一台电脑，这台电脑又不怎么换 IP 和 MAC 地址，只要记住这台电脑的 MAC 地址，如果目标 MAC 地址不是这台电脑的，这个口就不用转发了。谁能知道目标 MAC 地址是否就是连接某个口的电脑的 MAC 地址呢？这就需要一个能把 MAC 头拿下来，检查一下目标 MAC 地址，然后根据策略转发的设备，按第二节课中讲过的，这个设备显然是个二层设备，我们称为**交换机**。
...
机器多了，交换机也多了，就算交换机比 Hub 智能一些，但是还是难免有广播的问题，一大波机器，相关的部门、不相关的部门，广播一大堆，性能就下来了。就像一家公司，创业的时候，一二十个人，坐在一个会议室，有事情大家讨论一下，非常方便。但是如果变成了 50 个人，全在一个会议室里面吵吵，就会乱得不得了。

你们公司有不同的部门，有的部门需要保密的，比如人事部门，肯定要讨论升职加薪的事儿。由于在同一个广播域里面，很多包都会在一个局域网里面飘啊飘，碰到了一个会抓包的程序员，就能抓到这些包，如果没有加密，就能看到这些敏感信息了。还是上面的例子，50 个人在一个会议室里面七嘴八舌地讨论，其中有两个 HR，那他们讨论的问题，肯定被其他人偷偷听走了。那咋办，分部门，分会议室呗。

那我们就来看看怎么分。

- 物理分离，一个部门一个交换机，人少的部分浪费
- 另外一种方式是虚拟隔离，就是用我们常说的 VLAN，或者叫虚拟局域网，可以设置交换机每个口所属的 VLAN。如果某个口坐的是程序员，他们属于 VLAN 10；
...
有时候测试一个服务是否正常时，通常用 ping，但你知道如何工作的吗？

**ping 是基于 ICMP 协议工作的。ICMP 全称 Internet Control Message Protocol，就是互联网控制报文协议。**这里面的关键词是“控制”，那具体是怎么控制的呢？

ping 是两 部主机之间的回声与否判断，而traceroute可以追踪两部主机之间通过的各个节点 (node) 通讯状况的好坏呢

#### 抓包工具：

- 抓http协议层的报文，一般用fiddler和burpsuite。
- 看底层网络报文用wireshark好，能看见syn ack和完整的TCP报文，着重于协议分析
...
Wireshark（前称Ethereal）是一个网络封包bai分du析软件。网zhi络封包分析软件的功能是撷取网络封包，并尽可能显示出最为详细的dao网络封包资料。Wireshark使用WinPCAP作为接口，直接与网卡进行数据报文交换。

Burp Suite 是用于攻击web 应用程序的集成平台，包含了许多工具。Burp Suite为这些工具设计了许多接口，以加快攻击应用程序的过程。所有工具都共享一个请求，并能处理对应的HTTP 消息、持久性、认证、代理、日志、警报
wireshark是针对经过网卡的数据包进行监听，而burpsuite是用于web应用分析的工具。wireshark的使用就是直接选择网卡就可以抓包；而burpsuite则需要先在浏览器上设置代理，然后再启用它才能够捕捉到客户端发给web服务器的报文，而且这些报文会直接以明文的形式显示。

总的来说，burpsuite比wireshark更有侧重也更加细节，是专门用于web分析的工具。

#### burp suite简单使用

- burp suite社区版就足够用，还免费
- burp suite中主要使用proxy和intruder两个模块
- proxy模块默认监听127.0.0.1:8080（可以在options模块里修改），设置浏览器的代理服务器也是这个，就是当前浏览器的所有请求都会走proxy。走proxy有几种方式，只记录请求，intercept is off,如果on则请求会被阻断，forward阻断后放行，而drop则直接放弃响应。
- repeater模块是模拟重放的功能，比如一个请求执行一次以后，可以修改参数再次请求。
- intruder模块可以批量模拟重放，比如配置一系列的用户名或密码，暴力请求破解。模拟过程可以设置变量字典，比如用户名用哪个字典，密码用哪个字典，然后这个密码和用户名用哪种组合方式，是一一对应，还是一对多等挨个试

#### ping过程

如果在自己的可控范围之内，当遇到网络不通的问题的时候，除了直接 ping 目标的 IP 地址之外，还应该有一个清晰的网络拓扑图。并且从理论上来讲，应该要清楚地知道一个网络包从源地址到目标地址都需要经过哪些设备，然后逐个 ping 中间的这些设备或者机器。
...
ICMP 相当于网络世界的侦察兵。一种是主动探查的查询报文，一种异常报告的差错报文；ping 使用查询报文，Traceroute 使用差错报文。

Traceroute的目的是获取数据包途径的路由器，但使用的是UDP协议，为了能够收到回复，故意设置一个不存在的信息去访问目的服务器，目的服务器会报错。通过这个报错也就知道了可以到达这个服务器。。。
...
不改变 IP 地址的网关，我们称为转发网关；改变 IP 地址的网关，我们称为NAT 网关。(Network Address Translation，简称 NAT。其实这第二种方式我们经常见)
现在大家每家都有家用路由器，家里的网段都是 192.168.1.x，所以你肯定访问不了你邻居家的这个私网的 IP 地址的。所以，当我们家里的包发出去的时候，都被家用路由器 NAT 成为了运营商的地址了。

但是traceroute侦测的结果有时候会返回***，代表该 node 可能设有某些防护措施，让我们发送的封包信息被丢弃所致。 因为我们是直接透过路由器转递封包，并没有进入路由器去取得路由器的使用资源，所以某些路由器仅支持封包转递， 并不会接受来自客户端的各项侦测啦!此时就会出现上述的问题。因为 traceroute 预设使用 UDP 封包，如果你想尝试使用其他封包， 那么 -I 或 -T 可以试看看啰!

由于目前 UDP/ICMP 的攻击层出不穷，因此很多路由器可能就此取消这两个封包的 响应功能。所以我们可以使用 TCP 来侦测呦! 

例如使用同样的方法，透过等待时间 1 秒，以及 TCP 80 埠口的情况下，可以这样做:

```
traceroute -w 1 -n -T baidu.com
```
...
- 静态路由算法，其实就是路由表是手动控制，如何跳转的
- 动态路由算法，根据算法得出最短路径，其实就是很多网络，两个节点间如何走比较高效。
...
- 模拟信号，是传感器采集得到的连续变化的值
- 数字信号，将模拟信号量化（0，1）后得到的离散的值。由于计算机采用二进制，因此采用的是数字方式，同时数字信号只有两个值，不会出现含糊不清的时候，因此得以普遍在计算机中采用。

### 传输介质

计算机联网总是需要一个物理的介质，可以是同轴电缆，双绞线，光纤等有线介质，还包括电磁波，红外线等无线连接介质。

- 同轴电缆
- 双绞线，比一般导线低噪声干扰，抑制缆线内数据流动信号的衰减
- 光纤，为了实现同轴和双绞线不能实现的远距离连接，传送光信号，防止电磁干扰，实现高速传输。
- 无线，利用空气中的电磁波传输数据，和移动电话，电视机的遥控器一样不需要电缆。电磁波随波长的不同，性质也会不同。从短到长：r线，x线，紫外线，可见光，红外线，远红外线，微波，短波，中波，长波等不同用途的电磁波。微波以上的电磁波又统称为无线电波。

金属线则用于传送电信号，但信号损失、电磁干扰较大，同时维护成本也较 高。

在计算机网络的无线通信中经常使用的电磁波是红外线和微波，前者只适合短距离传输。在无线lan中利用2.4GHz的超短波频段进行通信，因为无线电波传播范围较广。

### 网络模型构成

- 网络接口卡（NIC，Network Interface Card）：也叫网络适配器、网卡、LAN卡;
- 中继器：处在OSI的物理层，物理层面上延长网络的设备，其实就是将电或光信号调整波形和放大再传给下一个电缆。
- 网桥：处在OSI的数据链路层，连接两个网络的设备。有些网桥能判断是否将数据报文转发给相邻的网段，这种网桥被称为自学式网桥。交换集线器（Hub）也是网桥的一种，交换集线器中连接电缆的每个端口都能提供类似网桥的功能。可以认为交换机的每个端口实际上提供着网桥的功能。
- 路由器在OSI的网络层，连接两个网络，并对分组报文进行转发的设备。网桥根据物理地址转化，而路由器则是根据ip地址进行处理。有的路由器不但可以分担网络负荷，还具备一定的网络安全的功能。
- 网关，负责从从传输层到应用层的数据进行传输和转发的设备，还负责数据转换，在两个不能直接通信的协议之间进行翻译，最终实现通信。有时为了客户端和服务端无需直接通信，中间加一个代理或者防火墙，这些其实都是网关的一种。
- IP（internet protocol）

### IP（internet protocol）网际协议

数据链路层只能在同一种数据链路的节点之间进行包传递（其实就是局域网），然后网络层可以跨域不同的数据链路。

- 主机，在互联网发明之初，只能连接大型的设备，因此习惯上将配有ip地址的设备称为主机，当然现在小型的也可以。
- 路由器，即有ip地址又有路由控制能力的设备
- 节点，是主机和路由器的总称，但一般范围是具有ip的。

在网桥或交换集线器等物理层或数据链路层数据包转发设备中，不需要ip地址，因为这些设备只负责将ip包转为0、1比特流或对数据链路帧的数据部分进行转发，而不需要应对ip协议。

- Hop又叫跳，ip路由器又叫多跳路由器，ip包正是在网络中一个跳间被转发，而跳的意思就是区间（相当于一段路）
- 不同网段内，允许传输的最大单位不同MTU（maxi-mum transmission unit），类似快递在不同的范围内有不同的快递限制。因此ip需要进行分片处理



面向连接 vs 面向无连接。面向连接，发送数据前，需要先建立连接。如果主机不存在或关机，或连接未建立均无法通信。反之即无连接。ip是面向无连接的，而tcp是面向连接的。

ipv4由32位组成，最大有2^32个IP地址，也就是43亿个。但IP地址并非按主机来配置，而是根据网卡，而且有时一个网卡还需要配置多个ip地址，而路由器通常还配置多个网卡。。。因此让43亿台计算机全部联网是不可能。其实ip地址还分为网络地址和主机地址，这样算下来，可用的数量会更少。

ip地址根据前四位的比特位，对网络标识和主机标识区分

- A类地址，首位以0开头，第1位到第8位是它的网络标识，0.0.0.0 ~ 127.0.0.0是A类地址
- B类地址，前两位以10开头，第1位到第16位是它的网络标识，128.0.0.1 ~ 191.255.0.0是B类地址
- C类地址，首位以110开头，第1位到第24位是它的网络标识，192.168.0.0 ~ 239.255.255.0是C类地址，此时一个网段内容纳的主机上限为254个（除掉特殊的两个，2^8 - 2 ）
- A类地址，首位以1110开头，第1位到第32位是它的网络标识，224.0.0.0 ~ 239.255.255.255是D类地址，该类地址没有主机标识，常用语广播

注意：主机地址不能全为0或全为1，全为1为广播地址，全为0一般在不知道ip地址时使用的特殊地址。

当然上面的几种是规定的，但网络端还可以自定义，毕竟经常情况下一个网段内的主机不多，要是用A或B类等则会严重浪费ip地址。此时可以自定义网络地址的长度，比如129.168.0.0/24，24就是表示前多少位为网络标识位。当然现在更常用的方式是通过子网掩码。

#### 广播地址

当主机标识全为1时，就为广播地址，129.168.0.0/24的广播地址为129.168.0.255，也就是最后8位为主机地址，全为1。这个广播地址的ip包会被路由器拦截，因为这个地址只在同一网段内有效。

本地广播：就是同一网段内广播
直接广播：不同网段内(首先会发送至路由器)
ip多播：将数据包同时发送给特定组内的所有主机(1对N或N对N)，本地广播无法穿透路由，但多播可以穿透路由，还可以只发送给需要的组。

#### 路由控制

路由控制表中存在多条相同网络地址的记录，会选择一条最吻合的一条，也就是相同位数最多的那条。例如：127.20.100.52的网络地址与127.20/16 和 127.20.100/24两项都匹配，此时应该选择后者。

**默认路由**，如果一张路由表中包含所有的网络及其子网的信息，将会造成无端浪费，此时可以用默认路由，指路由表中任何一个地址都与之匹配的记录。

默认路由一般标记为：0.0.0.0/0或default，注意此处的0.0.0.0/0并不是ip地址0.0.0.0，标识/0就是为了表明不是ip地址。

**主机路由**，IP地址/32 也叫主机路由，例如192.168.153.15/32就是一种主机路由。意思是整个ip地址都参与路由。而不再基于网路地址路由。

**回环地址**，同一台计算机上的程序之间进行网络通信时所使用的的一个默认地址。127.0.0.1与localhost主机名对应，使用回环地址，数据包不会流向网络。
...
说明：正常的数据包会从IP层进入链路层，然后发送到网络上；而给回环地址发送数据包，数据包会直接被发送主机的IP层获取，后面就没有链路层他们啥事了。

而127.0.0.1作为{127}集合中的一员，当然也是个回环地址。只不过127.0.0.1经常被默认配置为localhost的IP地址。

**0.0.0.0**，并不是一个真实的的IP地址，它表示**本机中所有的IPV4地址**。监听0.0.0.0的端口，就是监听本机中所有IP的端口。

#### IP分割处理与再构成处理

数据链路不同，MTU不同，而ip层属于数据链路的上一层，因此也必须分片才可以。而分片的操作是由路由器操作的，但负责组装的过程只能有目标主机，因为路由器会很多，若反复组装分片会很浪费性能。就好比买了一批货物，分批发货，最后自己整理。

然而后来为了降低丢包率以及减轻路由器的压力，还出现了 路径MTU发现（path mtu discovery），其实就是先找到整个通路中允许的最小的MTU，然后发送主机按照这个大小先分好片，然后中途的路由器就不会再分片了，遇到超过的，会直接丢弃。

#### IPv6

ipv6的地址长度是ipv4的4倍，也就是2^(32*4)，这将是一个天文数字，足够容纳人们所想到的所有主机和路由器部分。

ipv4升级后，为何直接叫ipv6呢？这里版本的命名不像我们平时的项目版本发布，互联网的版本迭代其实也是连续的，但互联网也重视实践，对于那些中间版本会反复试验，最后选择一个最佳的版本进行标准化，而ipv6正好是经历了这个过程后才能为ipv4下一代的ip协议的。

### IP协议相关技术

IP（internet protocol）旨在让最终目标主机收到数据包，但仅仅有ip还不够，必须还有能够解析主机名称和mac地址的功能，以及数据包在发送过程中异常情况下处理的功能。

#### DNS（Domain name System）

ip地址不便记忆，主机往往利用hosts的数据库文件映射IP地址与域名。最初的阿帕网中，由互联网信息中心（SRI-NIC）管理一份hosts文件，如果有变更或增加，则需要修改这个文件，同时其他计算机还需要定期下载更新hosts文件才行。但慢慢的，这种方式无法满足要求...

DNS便诞生了

**tip：**控制台：nslookup 域名，可以返回对应的ip地址。

域名也有属于自己的分层，比如某些主机名属于哪些组织？这些组织属于哪些国家等？其实dns的分层像一个倒挂的树，顶点是树的根，第一层有：uk英国，cn中国，jp日本等代表国家，和代表特定领域的域名（edu美国教育机构，com美国企业等）。其实类似公司的组织架构，公司下有各个部门，各个部门有不同的科室等等

**域名服务器**，负责管理所在分层的域的相关信息，每一层都设有一个域名服务器。根部所设置的DNS叫做根服务器，如果想修改类似jp或org的域名，则需要修改根服务器里的配置。

**解析器**，进行DSN查询的主机和软件叫做DNS解析器，工作站或个人电脑都有解析器。

DNS是通过主机名检索IP地址的系统，然而它管理的信息不仅仅是这些主机名与ip地址的映射关系，还管理其他众多信息。

例如，主机名与ip地址的对应信息叫**A记录**，反之，从IP地址检索主机名称的信息叫**PTR**，上层或下层域名服务器IP地址映射叫做**NS记录**。

#### ARP(address resolution protocol)

知道了ip，就可以向目标地址发送ip数据包，然而在数据链路层，进行通信时还需要该ip对应的mac地址。此时就用到了ARP协议。

不过ARP只适用于ipv4，ipv6的话需要使用ICMPV6。

**ARP工作机制**

主机a想要知道主机b的mac的地址，会通过广播发送一个ARP请求包，包里包含想要知道起mac地址主机ip地址，这样同一个链路内的主机或路由器都会接收到，如果请求包里的ip地址与自己的相同，那么当前节点就把自己的mac地址放到ARP响应里返回给主机a。（还会缓存下来）

#### RARP（reverse address resolution protocol）

是将ARP反过来，从MAC地址定位Ip地址的协议。平时使用嵌入式设备时，由于没有任何输入接口或无法通过DHCP（dynamic host configuration）动态获取ip地址的情况，此时就需要使用RARP，当然还需要RARP服务器。

#### ICMP

当需要确认网络是否正常，以及遇到异常时如何进行诊断，而ICMP就提供了这类功能的一种协议。

主要功能：确认ip是否成功送达目标地址，通知在发送过程中ip包被废弃的具体原因，网路改善设置等。

在ip通信中，如果某个ip包未能到达目标地址，那么具体原因由ICMP负责通知。

ICMP消息大致分为两类：一类是通知出错原因的错误信息，一类是用于诊断的查询消息。

- 0，回送应答（Echo reply）
- 3，目标不可达（Destination unreachable）
- 4，原点压抑（source quench）
- 5，重定向或改变路由（Redirect）
- 11，超时（time exceeded）
- 。。。

比如当返回目标不可达时，还会返回关于为何不可达的具体原因，如下：
...
**tip：**traceroute就是利用ICMP超时消息的应用，它可以显示主机到目标主机经历了多少路由器。原理就是利用ip包的生存期限从1开始按照顺序递增的同时发送UDP包，强制接收ICMP超时消息的一种方法。使用：traceroute 目标主机地址


**ICMP回送消息**，用于判断发送的数据包是否已经成功到达目标主机的一种消息。可以向目标发送回送请求的消息（类型8），也可以接收目标主机发回来的回送应答消息（类型0），ping（packet internetwork groper）命令就是利用这个消息实现的。


**DHCP（dynamic host configuation protocol）**

一般使用DHCP，首先要架设一台DHCP服务器，然后将DHCP所要分配的ip地址设置到服务器，此外还需要设置相应的子网掩码，路由控制信息以及DNS服务器的地址等。

一般情况下，该网段的路由器充当DHCP服务器。一般为了防止一台DHCP服务器挂掉影响全部，都会开启多个DHCP服务器，但多个后又会出现分配ip重复的问题，此时就需要在DHCP服务器上区分所要分配的地址。（感觉其实就是将一个局域网内的ip地址分组）

在家庭等小型局域网中，因为主机少，一般都只有个无线网段，一个DHCP足够了，多数时候由路由器担任。

但一个企业或一个学校，一般会有多个网段，此时要对每个网段设置DHCP将是一个庞大的工作。因此这类环境中，往往需要将DHCP统一管理，比如使用DHCP中继代理实现。

中继代理器过程：DCHP客户端会向DHCP中继代理器发送DHCP请求包，而代理器会将包以单播的方式给DHCP服务器，服务器收到包以后，再给代理器应答，并由代理器将包给DHCP客户端，由此，DHCP服务器即使不在同一个链路上也可以实现统一分配和管理ip地址。

#### NAT(network address translator)

用于在本地网络中，使用私有地址，在连接公网时转用公有ip地址的技术。还有可以转换TCP，UDP端口号的NAPT(network address ports translator)。由此来实现一个公网ip与多个主机的通信。

其实NAT技术正是为了环节ipv4地址短缺的问题出现。但ipv6中也可以使用

NAT的工作机制：在NAT（NAPT）路由器的内部，有自动生成的用来转换地址的表，当私有地址向目标地址请求时，会按照表中的映射处理。如果多个私有主机同时请求外网，仅仅转换ip地址还不够，因为转换完映射表完全一致，此时可以在结果映射中ip连同端口一块转（其实就是在结果映射表里，端口号递增来区分相同的请求）

在TCP首次握手时的SYN包一经发出，就会生成这个表，而后又随着收到关闭连接时发出FIN包的确认应答时从表中删除。

**NAT-PT（NAPT-PT）**

NAT是服务于ipv4，而为了服务ipv6，出现了NAT-PT将ipv6的首部转换为ipv4的首部的一种技术。有了这种技术，那些只有ipv6地址的主机就可以与ipv4的其他主机记性通信了。 
...
- 转换表的生成与转换需要开销
- 一旦NAT重启，所有的TCP连接被重置
- 即使备两台NAT做容灾，TCP连接还是会断开。

#### IP任播

IP任播主要用于110与119等报警系统，当人们拨打110等特殊号码时，其接收电话并不是一个，而是可以播到一个区域管辖内的所有公安或消防部门，所以数量极其庞大。这种机制就是IP任播。

IP任播：为那些提供同一种服务的服务器配置同一个IP地址，并与最近得服务器进行通信的一种方式。适用于ivp4和ipv6.

IP任播也用于DNS根服务器，由于历史原因，对ip地址的分类限制为13种类型，为此使用IP任播可以让更多的DNS服务器散布到世界各个角落。


#### Mobile IP

当移动设备进入到不同的网段内，按理说ip地址会发生变化，比如电脑的ip地址。但手机不同，因为再与移动设备通信时，子网网段一旦发生变化，则无法通过tcp继续通信，这是因为tcp是面向连接的协议，自始至终都需要发送端和接收端主机的IP地址不发生变化。因此Mobile IP技术保证所连接的子网发生变化时，主机IP地址仍然保持不变。（其实通过中间环节转换了一下）
...
作为服务端的程序有必要提前启动，否则收到客户端的请求，也无法做到处理。这些服务端程序再UNIX中叫守护进程，例如http的服务端程序是httpd（http守护进程），而ssh的服务端程序是sshd（ssh守护进程）。根据收到数据里的端口号，可以区分数据是发给哪个服务端程序的。

#### 

- TCP（Transmission control protocol）面向连接，提供可靠的流协议。还实行顺序控制或重发控制，还有流量控制，拥塞控制，提高网络利用率等功能
- UDP（User Datagram Protocol）常用于让广播和细节控制交给应用的通信传输。
- 二者应用的场景不同，前者要求面向连接，可靠且具备顺序控制，重发控制等机制。后者用于要求高速船速和实时性有较高要求的通信或广播通信。比如通话就是用udp，延迟小同时不需要重发，因为部分数据丢失，也不会影响到整个通话，在多播和广播中使用的也是UDP。还有包总量较少的通信，比如DNS，或者音视频媒体通信等。

UDP将应用程序发来的数据再收到的那一刻，立刻按照原样发送到网上的一种机制。即使网络拥堵时，udp也无法控制流量以避免网络拥塞。即使丢包也不负责重发。

UDP中的user，不单单指互联网的使用者，曾经也是为了表示哪些编写程序的程序员，也就是说udp是按照程序员的变成思想在传送数据。
...
- 目标端口号
- 包长度
- 校验和（为了提供可靠的udp首部和数据而设立）

#### TCP的特点及目的

连接是指各种设备、线路或网络中进行通信的两个应用程序为了相互传递消息而专有的、虚拟的通信线路，也叫虚拟电路。

tcp通过校验和、序列号、确认应答、重发机制、连接管理以及窗口控制等机制实现可靠性传输

两个人说话时，甲方说完，乙方一般会嗯或点头，同时甲方收到（看到）这个信号，表示已方已经收到信息。。。如果长时间甲方未收到这个信号，甲方就会再说一遍。。。这就类似TCP的重发机制

但未收到信号分多种：
- 数据压根没传到乙方，
- 还有就是乙方的确认收到信号在回来的路上丢包了。
- 还有就是因为网络延迟，在重发数据后，确认收到信号才回来。（此时目标主机会反复收到重复的数据，为了应对上层应用稳定的传输，必须放弃重复的包）

在目标主机反复收到重复的数据包时，就需要一种机制，能够识别是否已经接收数据，又能判断是否需要接收。这就需要序列号，序列号是按顺序给发送数据的每一个字节都标上号码的编号，接收端查询接收数据TCP首部中的序列号和数据的长度，将自己下一步应该接收的序列号作为确认应答返回去，就这样，通过序列号和确认应答号，tcp就可以实现可靠传输。

**重复超时的时间间隔？**

最理想的时间是找到一个最小值，然而网络环境的不同，这个时间会不同。tcp要求不论什么网络环境都要提供高性能通信，为此，他在每次发包时都会计算往返时间（Round Trip time RTT）以及偏差（RTT时间波动的值，方差），然后将这个往返时间和偏差相加，但是再稍微大一点。

当然，再次重发之后，如果还是收不到确认应答，则进行再次发送，等待确认应答的时间将会以2倍、4倍的指数函数延长。

如果一直没有没有收到应答，超过一定次数后，就会判断为网络发生异常，强制关闭连接，并且通知应用通信异常强行终止。

**连接管理**

三次握手：
1. 客户端发送建立连接的请求（SYN）（我要发请求啦）
2. 服务端针对SYN返回确认应答ACK（好的）
3. 再次发送ACK（那我现在开始发啦）

四次握手：
1. 客户端发送切断连接的请求（FIN）（我要断开连接啦）
2. 服务端针对FIN返回确认应答ACK（好的）
3. 服务端发送请求切断连接的FIN（那我也断开了啦）
4. 客户端针对FIN返回确认应答（好的）

**TCP以段为单位发送数据**

在建立tcp连接的同时，也同时确定了数据包发送的最大长度（MSS: Maximum segment size），最理想的情况是，最大消息长度正好是ip中不会被分片处理的最大数据长度。

获取长度的过程：
1. 客服端发送建立连接的请求，并把MSS设置为A；
2. 服务端返回确认应答，同时说把MSS设置B吧；
3. 客户端返回ACK，并说好的，那就把MSS设置为B了啊。

**利用窗口控制提高传输速度**

tcp收发数据包时，可以每个包都确认应答，如果10k，分10次传输，则需要应答10次，这就造成包的往返时间变长了。。。因此可以打包应答，比如4个一组再统一应答。发送端在发送完一个数据包后，不用等待应答，可以直接发送下一个数据包。这里的四个一组，其实就可以理解为一个窗口

如果一个窗口内，有一个数据包传输失败，则服务端总是会返回该序号的确认应答，发送端收到连续3次的同一个确认应答，就知道包传输失败，就会重新发送该数据包。

**流控制**

为了避免无端的网络流量浪费，tcp可以根据服务端的接收能力，控制发送的数据量。具体操作：接收端向发送端通知自己接收数据的大小，于是发送端发送的数据就不会超过这个值，该大小的限度就被称为窗口大小。tcp首部有专门字段来通知窗口大小。

**拥塞控制**

有了窗口控制，收发主机之间即使不再以一个数据段为单位发送数据，也能连续发送大量数据包，然而，如果在通信开始就发送大量数据，也可能会引发其他问题。

如果在网络拥堵的情况下，突然发送大量的数据，可能使得整个网络瘫痪，为了防止这个，通信开始会通过一个**慢启动的算法**得出数值，然后对发送数据量进行控制。

慢启动其实就是，开始发送端设置窗口大小为1，然后与服务端返回的大小对比，取二者中小值，之后每次收到一个确认应答，都会2^n次方调整窗口大小，依次为2、4、8...但又为了防止指数爆炸，有设置了最大拥塞窗口阈值。

tcp首部
- 源端口号
- 目标端口号
- 序列号，字段32位，指发送数据的位置，每发送一次数据，就累加一次该数据字节数的大小。需要一般不是从0或1开始，而是计算机随机生成的，通过SYN包床给接收端主机。
- 确认应答号，32位，指下一次应该收到的数据的序列号，其实也就是已经收到了该号减一为止的数据。
- 数据偏移，表示tcp所传输的数据部分应该从tcp包的哪个位置开始计算。其实也可以看做tcp首部的长度
- 保留
- 控制位，SYN,ACK等标识就在这个位上
- 窗口大小，26位，如果窗口大小为0，则表示发送的窗口探测，以了解最新的窗口大小。但这个数据必须是一个字节。
- 校验和，其实目的为了数据在传输过程中是否被破坏。
- 紧急指针


#### 套接字

应用在使用TCP和UDP时，会用到操作系统的类库，这种类库一般叫API。使用TCP和UDP时，又会广泛使用到套接字（socket，套接字之前用在UNIX上，后来移植到windows上）的API，利用套接字可以设置对端的ip地址，端口号，并实现数据的发送与接收。


#### 端口号

数据链路和ip中的地址，分别指的是mac地址和ip地址，前者识别同一链路中不同的计算机，后者用来识别TCP/IP网络中互连的主机和路由器。端口号用来识别同一台主机上不同的应用程序。

**端口号如何确定？**

- 标准既定的端口号，其实也就是静态方法，相当于一些有名的程序指定某些端口号，比如http的80，https的443，这些有名的端口号一般在0~1023之间（这也是为何尽量不要用），另外还有一些端口号也被注册，范围1024~49151。
- 时序分配法，其实就是操作系统自己给应用程序动态分配端口号，取值范围为49152~65535.

在小于 1024 的埠口，都是需要以 root 的身份才能启动的，这些 port 主要是用于一些常见的通讯服务，在 Linux 系统下，常见的协议与 port 的对应是记录在 /etc/services 里面的

**端口号和协议**

不同的传输协议，可以使用相同的端口号，比如tcp和udp都可以使用53.

### 应用层协议

#### telnet

telnet 是利用tcp的一条连接，经常用于登录路由器或高性能交换机等网络设备进行相应的设置，同时登录前还需要在服务器将登录名和密码注册到服务器。telnet也可用于跟踪tcp/ip应用开发阶段的问题诊断

#### ssh

ssh是加密的远程登录系统，而telnet登录时无需密码也可以发送，容易造成通信窃听或非法入侵，另外就是telnet是明文传输，而ssh是加密传输。

- 可以使用更强的认证机制
- 可以转发文件（scp，sftp等）
- 可以使用端口转发（指特定端口收到的信息转发到指定ip和端口的机制）

1. 服务器建立公钥档: 每一次启动 sshd 服务时，该服务会主动去找 /etc/ssh/ssh_host* 的档案，若系统刚刚安装完成时，由于没有这些公钥档案， 因此 sshd 会主动去计算出这些需要的公钥档案，同时也会计算出服务器自己需 要的私钥档;

2. 客户端主动联机要求:若客户端想要联机到 ssh 服务器，则需要使用适当 的客户端程序来联机，包括 ssh, pietty 等客户端程序;
...
在我们使用的 Linux 系统当中，默认就已经含有 SSH 的所有需要的软件
了!这包含了可以产生密码等协议的 OpenSSL 软件与 OpenSSH 软件

openssl 命令行工具，通过它来检查整个握手和本地服务器配 置情况:
...
ftp是计算机间数据进行传输的协议，允许任何人访问。

工作机制：使用两条tcp连接，一条用来控制（端口21），一条用于文件传输（端口20），

**电子邮件的工作机制**

提供电子邮件服务的协议SMTP（simple mail transfer protocol）。早期的电子邮件只是简单的建立tcp连接，然后通信，这样受限很多（比如必须两台主机同时在线等），后来出来了一直在线的邮件服务器，主机之间不在直接通信，而是通过邮件服务器，这样就可以随时随地的发送和接收邮件，而不用担心对方主机的在线状态。

邮箱名类似域名，也是需要dns解析的，而邮箱地址与邮箱服务器的映射关系就是MX记录。

MIME，早期邮件只能发送文本格式，但现在可以发送MIME里定义的格式。

SMTP是发送电子邮件的协议，使用tcp的25端口，由于SMTP本身并没有验证发送者的功能，因此无法避免垃圾邮件到达自己的邮件服务器，不过现在通过 pop before smtp或SMTP认证，可以防止冒充发送者的人。

可以使用telnet登录SMTP服务器，如：telnet 服务器名或ip地址 25

**POP**

我们知道SMTP需要借助邮件服务器才可以不关心发送端和接收端的状态，而pop就是一种具体的方式，发送端根据SMTP协议将邮件发送给一直插电的POP服务器，接收端再根据POP协议从POP服务器接收对方发来的邮件。

**IMAP（internet message access protocol）**

与POP类似也是接收邮件的协议，POP中邮件由客户端进行管理，IMAP是在服务器进行管理


**多媒体通信实现技术**

我们知道通话或电视会议使用UDP协议，但是只使用UDP协议还不够，还需要SIP，RTP和压缩技术等。

- SIP(session initiation protocol)协议，终端之间进行多媒体通信时，需要事先解析对方地址、呼出对方号码并对所要传输的媒体信息进行处理等功能，此外，还需要具备终端回话和数据转发的功能，这些功能都被统一于SIP协议中。
- RTP(real-time protocol)，udp不是一种可靠性传输协议，因此可能会丢包或乱序问题，因此用udp实现实时的多媒体通信需要附加一个表示报文顺序的序列号字段，还需要对报文发送时间进行管理，这就需要RTP。
- 数字压缩技术，MPEG(moving picture experts group)是决定数字压缩规范的工作组，平时用到的MPEG4就是4版本。

**P2P（peer to peer）**

电子邮件属于一台服务器对应多个客户端的c/s模型，即1对n的通信形态。与之相反，还有种1对1的通信模式p2p，p2p中的主机需要同时具备客户端和服务端两方面的功能。 比如文件传输服务bitTorrent协议使用了p2p技术。

但使用NAT的网络中，p2p是行不通的，因为p2p要求两端的主机同时具备直接访问公网的能力，通过NAT转换的不可以。
...
- URI(Uniform resource identifier)资源标识，更加广义
- URL（uniform resource locator）资源的具体位置
- 信息的表现形式（html，hyperText Markup language）
- 信息的转发（http，hyperText transfer protocol）

XML（extensible markup language）是从SGML（standard generalized markup language）衍生出来的一种语言。(其实这是在计算机里语言，比如汉语里的汉字)

RSS（really simple syndication）site summary，其实就是抽取的网页摘要，把一些关键信息抽离出来形成RSS，当想快速从很多信息里获取数据时，可以使用RSS。

### 网络安全

起初，TCP/IP只用于一个相对封闭（一个很小众的）的环境，之后慢慢发展壮大并普及，发生了很多非法访问和恶意攻击等问题，所以安全才慢慢重视起来。
...
防火墙种类有很多种，例如，专门过滤（不过滤）特定数据包的包过滤防火墙，数据到达应用以后由应用处理并拒绝非法访问的应用网关。

- 比如给路由器设置包过滤防火墙，只允许发送特定地址和端口号的包
- 从外部过来的tcp通信请求时，只允许对web服务器的tcp 80端口和邮件服务器的tcp 25端口的访问，其他所有类型的包全部丢弃。
- 还可以设置tcp连接的请求只允许从内网发起，可以通过设置SYN和ACK标志位来实现，当SYN=1，ACK=0时属于互联网发过来的包，应当废弃。

#### IDS（入侵检测系统）

数据包符合要求会穿过防火墙，但无法检测是否合法，此时就需要配合IDS，IDS定期采集日志，长期监控，通知异常等功能，它可以监听网络上流动的数据包，因此可以与防火墙相辅相成。


#### 反病毒/个人防火墙

反病毒/个人防火墙是继IDS和防火墙之后的另外两种安全策略，他们往往是用户使用的计算机或服务器上运行的软件。既可以监听计算机中进出得的所有包、数据和文件，也可以防止对计算机的异常操作和病毒入侵。

#### 安全协议

以前，为了防止信息泄露，对机密数据的传输，一般不适用互联网等公共网络，而是使用由专线连接的私有网络（private network），从而在物理上杜绝窃听和篡改数据的可能，然而专线造价高昂。

为此，人类在互联网构造了一个虚拟的私有网络，即VPN(vitrual private network，虚拟转用网络)，采用加密和认证技术达到即使读取到数据也无法读懂，检查是否被篡改等功效。vpn也正是使用这两种技术。
...
数据传输中发生异常，怎么排查呢，可以检查ICMP包，或利用traceroute等命令定位发生异常的设备或线路最近得ip地址，然后找到这个ip地址的管理员，提出问题并找到问题突破口。

但如何知道这个ip地址属于哪个机构呢？可以在https://who.is/whois/上查询。其实相当于找到这个公网ip是哪个机构使用的。

- netstat查看关于网络的一些状态，比如哪些端口正在开启着服务，哪些有问题？
- host [-a] hostname ，-a列出该主机详细的各项主机名设定数据，可以查到对应域名的ip地址。

网络参数设定使用的指令
5.1.1 手动/自动设定与启动/关闭 IP 参数:ifconfig, ifup, ifdown 5.1.2 路由修改: route
5.1.3 网络参数综合指令: ip
5.1.4 无线网络: iwlist, iwconfig
5.1.5 手动使用 DHCP 自动取得 IP 参数:dhclient
5.2 网络侦错与观察指令
5.2.1 两部主机两点沟通: ping, 用 ping 追踪路径中的最大 MTU 数值 5.2.2 两主机间各节点分析: traceroute
...
5.4 文字接口网页浏览
5.4.1 文字浏览器: links 5.4.2 文字接口下载器: wget
5.5 封包撷取功能
...
MIME最初是为了解决在不同的电子邮件系统之间搬移报文时存在的问题，因为工作非常好，后来http采纳了它。服务器会为所有web资源添加mime类型，然后浏览器拿到资源后，会根据MIME类型来判断，是否可以处理这个资源。

MIME是一种文本标记，有对象类型和一个特定的子类型构成，中间斜杠，如：
...
URL是可以移植的，它统一地表示因特网上所有的资源，这就意味着要通过各种不同的协议来传送这些资源，这写协议在传输数据时都会使用不同的机制，所以，设计url，使其可以通过任意因特网协议安全第传输是很重要的。

安全传输意味着url的传输信息不能丢失。。。有些协议在传输时会剥去一些特定的字符（因为报文是7位二进制编码，如果源端是以8位或更多位编码的，就会有部分信息被剥离），为了避开这些问题，只能使用一些相对较小的，通用的安全字母表中的字符。

另外设计者们还希望URL是可读的，因此即使不可见，不可打印的字符能够穿过邮件程序，从而成为可移植的，也不能在URL中使用。URL还得是完整的，人们可能希望URL中包含除完全字母表外的二进制数据或字符，因此需要一种转义机制，将不安全的字符编码为安全字符，再进行传输。

编码机制：一个百分号，后面跟着两个表示字符ASCII码的十六进制数。

- PUT，让服务器用请求的请求体部分来创建一个由所请求的url命名的新文档？如果存在，则替换。。。前端定义的东西，然后变成服务器的？因为PUT允许用户对内容修改，因此服务器在执行PUT之前，需要登录。
- TRACE，客户端发起请求时，肯能要穿过防火墙，代理，网关等，每个中间环节都可能改变http的请求，trace请求会在目的服务器上发起一个环回诊断，其实就是最后一站，会返回一条trace响应，并在响应中携带它收到的原始请求报文，这样客户端就可以查看原始请求报文是否被修改过。(但http库不一定支持。。。)
- OPTIONS请求，其实就是问服务器你支持哪些方法啊，或者对某些特殊资源支持哪些方法啊。其实就是不用实际访问这些资源，就能判定访问各种资源的最优方式。在与服务器进行交互之前，确定服务器的能力
- delete，其实就是让服务器删除请求url所制定的资源，但服务器可以不执行，同时还可以不告知浏览器。

状态码：

- 100 ~ 199为信息类状态码，如果请求头里带有100 continue的expect首部，意思是：客户端想发送一个实体给服务器，但希望在发送之前服务器查看一下是否会接收这个实体。
- 200 ~ 299成功状态码，204 no content，响应报文中包含若干首部和一个状态行，但没有实体的主题部分。
- 300 ~ 399重定向状态码，告知客户端使用替代位置来访问他们要的资源，要么提供一个替代的响应而不是资源的内容，如果资源被移动，则发送一个重定向状态码和一个可选的location首部，告知客户端资源移到这个位置了。301永久，302临时，304未修改，307与301类似
- 400 ~ 499客户端错误状态码，其实就是客户端发送一些服务器无法理解的东西，比如错误的报文，错误的url地址，很多客户端错误时浏览器处理的，比如404。
- 500 ~ 599服务器错误状态码，500服务器服务挂了，502bad gateway网关收到一条错误响应。503服务暂时不可用。504gateway timeout网关或代理在等待另一个服务响应时超时了。


#### 通用首部

首部和方法配合，决定客户端和服务器能做什么事情。

- Date，说明报文是什么时间创建的
- Transfer-Encoding，告知接收端为了报文的可靠传输，对报文采用了什么编码方式
...
- http/1.0+ keep-alive，现在新规范已经不再使用了，现在使用的只是兼容之前的
- http/1.1 persistent

keep-alive只是value，发送时key为connection，客户端请求头携带Connection：keep-alive，然后如果服务器支持并且愿意为下一个请求保持长连接，可以继续在响应头里返回Connection：keep-alive。另外keep-alive还可以作为key，说明长连接的具体属性：timeout服务器希望将连接保持的活跃时间，max是还希望多少个请求保持长连接，还有其他的属性。
...
connection)的改进型设计取代了它。持久连接的目的与 keep-alive 连接的目的相 97 同，但工作机制更优一些。
...
严格来说，代理连接的是两个或多个使用相同协议的应用程序，而网关连接的则是 两个或多个使用不同协议的端点。实际上，代理和网关之间的区别很模糊。由于浏览器和服务器实现的是不同版本的 HTTP，代理也经常要做一些协议转换工作。而商业化的代理服务器也会实现网关 的功能来支持 SSL 安全协议、SOCKS 防火墙、FTP 访问，以及基于 Web 的应用程 序。

- Proxy Auto-Configuration，PAC代理的自动配置，提供一个 URI，指向一个用 JavaScript 语言编写的代理自动配置文件;客户端会 取回这个 JavaScript 文件，并运行它以决定是否应该使用一个代理，如果是的 话，应该使用哪个代理服务器。
- WP AD 的代理发现
有些浏览器支持 Web 代理自动发现协议(Web Proxy Autodiscovery Protocol，WPAD)，这个协议会自动检测出浏览器可以从哪个“配置服务器”下载到一个自动配置文件。
- via 首部字段列出了与报文途经的每个中间节点(代理或网关)有关的信息。报文每经过一个节点，都必须将这个中间节点添加到 Via 列表的末尾。请求和响应报文都会经过代理进行传输，因此，请求和响应报文中都要有 Via首部。
- 当 TRACE 请求到达目的服务器时，16 整条请求报文都会被封装在一条 HTTP 响应 的主体中回送给发送端(参见图 6-23)。当 TRACE 响应到达时，客户端可以检查 服务器收到的确切报文，以及它所经过的代理列表(在 Via 首部)。TRACE 响应的 Content-Type 为 message/http，状态为 200 OK。通常，不管中间插入了多少代理，TRACE 报文都会沿着整条路径传到目的服务器 上。可以使用 Max-Forwards(最大转发次数)首部来限制 TRACE 和 OPTIONS 请求所经过的代理跳数，
- 代理服务器可能无法理解所有经其传输的首部字段。有些首部可能比代理自身还要
新;其他首部可能是特定应用程序独有的定制首部。代理必须对不认识的首部字段进行原样转发。

### 缓存

- 缓存减少了冗余的数据传输，节省了你的网络费用，比如重复数据。
- 缓存缓解了网络瓶颈的问题。不需要更多的带宽就能够更快地加载页面。
- 缓存降低了对原始服务器的要求。服务器可以更快地响应，避免过载的出现
- 缓存降低了距离时延，因为从较远的地方加载页面会更慢一些。即使带宽不是问题，距离也可能成为问题。每台网络路由器都会增加因特网流量的 时延。即使客户端和服务器之间没有太多的路由器，光速自身也会造成显著的时延。

HTTP 为我们提供了几个用来对已缓存对象进行再验证的工具，但最常用的是 If- Modified-Since 首部。将这个首部添加到 GET 请求中去，就可以告诉服务器， 只有在缓存了对象的副本之后，又对其进行了修改的情况下，才发送此对象。


TTP 没有为用户提供一种手段来区分响应是缓存命中的，还是访问原 始服务器得到的。在这两种情况下，响应码都是 200 OK，说明响应有主体部分。客户端有一种方法可以判断响应是否来自缓存，就是使用 Date 首部。将响应中 Date 首部的值与当前时间进行比较，如果响应中的日期值比较早，客户端通常就可以认为这是一条缓存的响应。客户端也可以通过 Age 首部来检测缓存的响应，通过 这个首部可以分辨出这条响应的使用期(参见附录 C 中的 Age 首部)。

通过特殊的 HTTP Cache-Control 首部和 Expires 首部，HTTP 让原始服务器向每个文档附加了一个“过期日期”。就像一牛奶上的过期日期那样，这些首部说明了在多长时间内可以将这些内容视为新鲜的。

- Expires，指定一个绝对的过期日期，其实就是未来哪一天到期。
- Cache-control：max-age=432000，单位s，

对缓存再验证来说最有用的 2 个首部是 If-Modified-Since 和 If-None-Match，所有的条件首部都是以前缀If-开头。也就是客户端携带的请求头。
...
HTTP 允许用户对被称为实体标签(ETag)的“版本标识符” 进行比较。实体标签是附加到文档上的任意标签(引用字符串)。它们可能包含了文 档的序列号或版本名，或者是文档内容的校验和及其他指纹信息。

如果服务器上的实体标签已经发生了变化(可能变成了 v3.0)，服务器会在一个 200 OK 响应中返回新的内容以及相应的新 Etag。其实Etag就是服务器给浏览器的一个标识，浏览器下次拿着这个标识，去问服务器是否更新，若没有更新返回304，同时Etag没有发生改变。若更新了，则更新Etag，同时响应200

HTTP/1.1 中提供 Pragma: no-cache 首部 17 是为了兼容于 HTTP/1.0+。除了与只理解 Pragma: no-cache 的 HTTP/1.0 应用程序进行交互时，HTTP 1.1 应用程序都 应该使用 Cache-Control: no-cache。

**隧道：**

Web 隧道允许用户通过 HTTP 连接发送非 HTTP 流量，这样就可以在 HTTP 上捎带 其他协议数据了。使用 Web 隧道最常见的原因就是要在 HTTP 连接中嵌入非 HTTP 流量，这样，这类流量就可以穿过只允许 Web 流量通过的防火墙了。最初开发 Web 隧道是为了通过防火墙来传输加密的 SSL 流量。很多组织都会将所有 流量通过分组过滤路由器和代理服务器以隧道方式传输，以提升安全性。但有些协 议，比如加密 SSL，其信息是加密的，无法通过传统的代理服务器转发。隧道会通过 一条 HTTP 连接来传输 SSL 流量，以穿过端口 80 的 HTTP 防火墙

HTTP 中继(relay)是没有完全遵循 HTTP 规范的简单 HTTP 代理。中继负责处理 HTTP 中建立连接的部分，然后对字节进行盲转发。HTTP 很复杂，所以实现基本的代理功能并对流量进行盲转发，而且不执行任何首 部和方法逻辑，有时是很有用的。

### web机器人

Web 机器人是能够在无需人类干预的情况下自动进行一系列 Web 事务处理的软件 程序。很多机器人会从一个 Web 站点逛到另一个 Web 站点，获取内容，跟踪超链， 并对它们找到的数据进行处理。根据这些机器人自动探查 Web 站点的方式，人们 为 它 们 起 了 一 些 各 具 特 色 的 名 字 ， 比 如 “ 爬 虫 ”、“ 蜘 蛛 ”、“ 蠕 虫 ” 以 及 “ 机 器 人 ” 等，就好像它们都有自己的头脑一样。

### cookie

产生 cookie 的服务器可以向 Set-Cookie 响应首部添加一个 Domain 属性来控制哪些站点可以看到那个 cookie。比如，下面的 HTTP 响应首部就是在告诉浏览器将 267 cookie user="mary17" 发送给域 ".airtravelbargains.com" 中的所有站点:
Set-cookie: user="mary17"; domain="airtravelbargains.com"


cookie 规范甚至允许用户将 cookie 与部分 Web 站点关联起来。可以通过 Path 属性来实现这一功能，在这个属性列出的 URL 路径前缀下所有 cookie 都是有效的。

例如，某个 Web 服务器可能是由两个组织共享的，每个组织都有独立的 cookie。 站点 www.airtravelbargains.com 可能会将部分 Web 站点用于汽车租赁——比如， http://www.airtravelbargains.com/autos/——用一个独立的 cookie 来记录用户喜欢的 汽车尺寸。可能会生成一个如下所示的特殊汽车租赁 cookie:
...
• 固定宽度 固定宽度方式的编码用固定数量的比特表示每个编码后的字符。它们能被快速处 理，但可能会浪费空间。
• 可变宽度(无模态) 可变宽度方式的编码对不同的字符代码数字采用不同数量的比特。对于常用字 符，这样可以减少需要的位数，而且还能在允许使用多字节来表示国际性字符的 同时，保持对传统 8 位字符集的兼容性。
• 可变宽度(有模态) 有模态的编码使用特殊的“转义”模式在不同的模态之间切换。例如，可以用有 模态的编码在文本中使用多个互相有重叠的字符集。有模态的编码处理起来比较 复杂，但它们可以有效地支持复杂的书写系统。

在下面列出的这些时间段内，网站通常是无法运作的。
• 服务器宕机的时候。
• 交通拥堵:突然间很多人都要看某个特别的新闻广播或涌向某个大甩卖网店。突
然的拥堵可以使 Web 服务器过载，降低其响应速度，甚至使它彻底停机。 • 网络中断或掉线。

服务器集群是一排配置相同的 Web 服务器，互相可以替换。每个服务器上的内容可 以通过镜像复制，这样当某个服务器出问题的时候，其他的可以顶上。

简单地说，内容分发网络(CDN)就是对特定内容进行分发的专门网络。这个网络 中的节点可以是 Web 服务器、反向代理或缓存。

虽然 CDN 最常用于在全球优化分发静态资源，但其优点并不止于此。距离客户端 更近的服务器还可以缩短 TLS 会话，因为 TCP 和 TLS 握手的对象都是近处的服务 器，所以建立连接的总延迟就会显著减少。简言之，把服务器放到接近客户端的地方能够节约 TCP 和 TLS 握手的时间!

Transfer-Encoding 首部与 Content-Encoding 首部不同，因为服务器或其他中 间应用程序是通过执行 Transfer-Encoding 对要传输的报文进行编码的。
...
对以太网来说，媒体就是物理线缆，对 Wi-Fi 而言，媒体则是无线信道。
...
我们看到的网络提供者 (Internet Services Provider, ISP) 所宣 称他们的 ADSL 传输速度可以达到 下行/上行 2Mbps/128Kbps (Kbits per second) 时，那个 Kb 指的可不是 bytes 而是 bits 喔!所以 2M/128K 在实际的档案大小传输速度上面，最大理论的传 输为 256KBps/16 KBps(KBytes per second)，所以正常下载的速度 约在每秒 100~200 KBytes 之间吶! 同样的道理，在网络卡或者是 一些网络媒体的广告上面，他们都会宣称自己的产品可以自动辨识 传输速度为 10/100 Mbps ( Mega-bits per second)，呵呵!该数 值还是得再除以 8 才是我们一般常用的档案容量计算的单位 bytes 喔!
...
每一部主机都有自己的路由表喔!观察路由表的指令很简单，就是 route
...
### 网络安全与主机基本防护: 限制端口口, 网络升级与 SELinux

要进入 Linux 本机的封包都会先通过 Linux 核心的预设防火墙，就是称 为 netfilter 的咚咚，简单的说，就是 iptables 这个软件所提供的防 火墙功能。为何称为封包过滤呢?因为他主要是分析 TCP/IP 的封包表头 来进行过滤的机制，主要分析的是 OSI 的第二、三、四层，主要控制的 就是 MAC, IP, ICMP, TCP 与 UDP 的埠口与状态 (SYN, ACK...) 等。
...
预设的防火墙是 Linux 的内建功能，但防火墙主要管理的是 MAC, IP, Port 等 封包表头方面的信息，如果想要控管某些目录可以进入， 某些目录则无法使用 的功能，那就得要透过权限以及服务器软件提供的相关功能了。举例来说，你可 以在 httpd.conf 这个配置文件之内规范某些 IP 来源不能使用 httpd 这个服 务来取得主机的数据， 那么即使该 IP 通过前面两层的过滤，他依旧无法取得 主机的资源喔!但要注意的是， 如果 httpd 这支程序本来就有问题的话，那么 client 端将可直接利用 httpd 软件的漏洞来入侵主机，而不需要取得主机内 root 的密码!因此， 要小心这些启动在因特网上面的软件喔!

为了避免前面一个步骤的权限误用，或者是程序有问题所造成的资安状况，因此 Security Enhanced Linux (安全强化 Linux) 就来发挥它的功能啦!简单的说， SELinux 可以针对网络服务的权限来设定一些规则 (policy) ，让程序能够进行 的功能有限， 因此即使使用者的档案权限设定错误，以及程序有问题时，该程 序能够进行的动作还是被限制的，即使该程序使用的是 root 的权限也一样。举 例来说，前一个步骤的 httpd 真的被 cracker 攻击而让对方取得 root 的使用 权，由于 httpd 已经被 SELinux 控制在 /var/www/html 里面，且能够进行的 功能已经被规范住了，因此 cracker 就无法使用该程序来进行系统的进一步破 坏啰。现在这个 SELinux 一定要开启喔!

rootkit 意思是说可以取得 root 权限的一群工具组 (kit)，就如同前面主动攻击 程序漏洞的方法一样， rootkit 主要也是透过主机的程序漏洞。不过， rootkit 也会 透过社交工程让用户下载、安装 rootkit 软件， 结果让 cracker 得以简单的绑架对 方主机啊!

封包进入本机时，会通过防火墙、服务器 软件程序、SELinux 与文件系统等。

- uname -r 可以查看系统版本

NFS 就是 Network FileSystem 的缩写，最早之前是由 Sun 这家公司所发展出来 的 (注 1)。 它最大的功能就是可以透过网络，让不同的机器、不同的操作系统、可以 彼此分享个别的档案 (share files)。所以，你也可以简单的将他看做是一个文件服务 器 (file server) 呢!

因为预设 NFS 用来传输的埠口是随机选择小于 1024 以下的埠口来使 用的。咦!那客户端怎么知道你服务器端使用那个埠口啊?此时就得要 远程过程调用 (Remote Procedure Call, RPC) 的协议来辅助啦!

因为 NFS 支持的功能相当的多，而不同的功能都会使用不同的程序来启动， 每启 动一个功能就会启用一些端口口来传输数据，因此， NFS 的功能所对应的端口口才没 有固定住， 而是随机取用一些未被使用的小于 1024 的埠口来作为传输之用。但如此 一来又造成客户端想要连上服务器时的困扰， 因为客户端得要知道服务器端的相关埠 口才能够联机吧!

此时我们就得需要远程过程调用 (RPC) 的服务啦!RPC 最主要的功能就是在指定 每个 NFS 功能所对应的 port number ，并且回报给客户端，让客户端可以连结到正确 的埠口上去。 那 RPC 又是如何知道每个 NFS 的埠口呢?这是因为当服务器在启动 NFS 时会随机取用数个埠口，并主动的向 RPC 注册，因此 RPC 可以知道每个端口口对 应的 NFS 功能，然后 RPC 又是固定使用 port 111 来监听客户端的需求并回报客户端 正确的埠口， 所以当然可以让 NFS 的启动更为轻松愉快了!

由于 NFS 的各项功能都必须要向 RPC 来注册，如此一来 RPC 才能了解 NFS 这个 服务的各项功能之 port number, PID, NFS 在服务器所监听的 IP 等等，而客户端才 能够透过 RPC 的询问找到正确对应的埠口。 也就是说，NFS 必须要有 RPC 存在时才 能成功的提供服务，因此我们称 NFS 为 RPC server 的一种。事实上，有很多这样的 服务器都是向 RPC 注册的，举例来说，NIS (Network Information Service) 也是 RPC server 的一种呢。

### DNS服务器

dig +trace www.baidu.com，让大家瞧瞧整个 DNS 的搜寻过程!在 dig 加上 +trace 的选项后，就能够达到这 个目的。

好了，既然 DNS 系统使用的是网络的查询，那么自然需要有监听的 port 啰!没 错!很合理!那么 DNS 使用的是那一个 port 呢?那就是 53 这个 port 啦!你可以 到你的 Linux 底下的 /etc/services 这个档案看看!搜寻一下 domain 这个关键词， 就可以查到 53 这个 port 啦!
...
SSL(Secure Sockets Layer，安全套接字层)协议最初是网景公司为了保障网上交易安全而开发的，该协议通过加密来保护客户个人资料，通过认证和完整性检查来 确保交易安全。
...
TLS 协议的目标是为在它之上运行的应用提供三个基本服务:加密、身份验证和数 据完整性
