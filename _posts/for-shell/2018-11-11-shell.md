...
export PATH=/usr/local/bin:$PATH这条命令的作用是将/usr/local/bin目录添加到当前用户的环境变量PATH中，并将原有的PATH环境变量的值追加在/usr/local/bin的后面。换句话说，这条命令将/usr/local/bin目录放到了PATH环境变量的最前面，使终端在执行命令时先从/usr/local/bin中查找命令或程序，如果没有找到再继续从原有的PATH环境变量中查找。

在Unix和Linux系统中，PATH环境变量是一个由多个目录组成的列表，用冒号(:)分隔。当用户在终端中输入一个命令时，终端会在PATH环境变量中列出的每个目录中查找命令或程序，直到找到与命令名相匹配的程序为止。如果在PATH环境变量中找不到相应的程序，则会出现"command not found"的错误提示。

在这条命令中，$PATH表示当前的PATH环境变量的值，用冒号(:)分隔多个目录。将/usr/local/bin放在$PATH的前面，就可以优先查找/usr/local/bin目录下的命令或程序。export命令的作用是将变量设置为环境变量，这样子进程和子shell也能够继承这个环境变量。

综上所述，这条命令的作用是将/usr/local/bin目录添加到PATH环境变量的最前面，使终端先从/usr/local/bin目录中查找命令或程序。


***系统的大小写**

在 Git 中，默认情况下，文件名是大小写敏感的，这意味着 file.txt 和 File.txt 会被视为两个不同的文件。但是，如果你的操作系统（如 Windows 或者某些文件系统）是大小写不敏感的，Git 会根据操作系统的默认行为来处理文件名。

你可以设置 core.ignoreCase 配置项

**环境变量的读取优先级**：  
在 Linux 或 macOS 系统中，多个配置文件设置了相同的环境变量时，会按照一定的优先级顺序进行覆盖。具体来说，优先级顺序如下：

命令行参数优先级最高：如果在命令行中直接指定了环境变量，那么该值将优先覆盖其他设置。

/etc/profile 文件中的 PATH 设置：这是全局环境变量的默认设置，如果没有其他的设置，将使用该值。

/etc/paths 文件中的 PATH 设置：这个文件也包含了一些系统级别的路径设置，会在 /etc/profile 之后被加载。

~/.bash_profile 或 ~/.bashrc 文件中的 PATH 设置：如果用户在个人的 Bash shell 配置文件中设置了 PATH 环境变量，将会覆盖 /etc/paths 文件中的设置。

/etc/bashrc 文件中的 PATH 设置：这是一个全局的配置文件，其中包括了一些系统级别的 Bash shell 配置信息，会在个人配置文件之后被加载。

需要注意的是，个人配置文件的优先级高于系统级别的配置文件，即用户自定义的设置会覆盖系统默认设置。而且，这些配置文件的加载顺序可能会因操作系统版本和用户环境而有所不同。
...
➜  ~ echo $path
/usr/local/bin /usr/bin /bin /usr/sbin /sbin /Users/yq/.cargo/bin
...
这是一个 Bash shell 配置文件（通常是 /etc/profile 文件）中的一段代码片段，主要用于设置 PATH 环境变量和加载 Bash shell 配置信息。

这段代码中包括两个主要部分：

PATH 环境变量设置：代码的第一行使用条件语句检查 /usr/libexec/path_helper 文件是否存在并且可执行，如果是，则执行该文件并将其输出解析为 PATH 环境变量设置。path_helper 是一个 macOS 系统工具，用于管理系统级别的环境变量设置，包括 PATH 环境变量。这行代码的作用是将 PATH 环境变量设置为 /usr/libexec/path_helper 输出的路径列表。

Bash shell 配置信息加载：代码的第四行使用条件语句检查是否运行在 Bash shell 环境中，如果是，则加载 /etc/bashrc 文件中的配置信息。/etc/bashrc 文件包含了一些系统级别的 Bash shell 配置信息，例如自定义别名、环境变量设置等。这行代码的作用是在 Bash shell 环境中加载该文件中的配置信息。

总体来说，这段代码的作用是设置 PATH 环境变量，以及在 Bash shell 环境中加载一些系统级别的配置信息。这些设置可以让用户在 Bash shell 环境中更方便地使用系统资源和执行命令。
...
我本机没有，~/.bash_profile 或 ~/.bashrc，而/etc/bashrc 也类似 /etc/profile 里的产物
...
# 如果一个全局安装的库，提示cmd not found 怎么办？
# 其实要明白，一个包安装成功后，可以通过绝对路径去访问的，
# 查看npm 全局安装的目录：
npm root -g           # /Users/dd/.nvm/versions/node/v14.15.0/lib/node_modules
npm config get prefix # /Users/dd/.nvm/versions/node/v14.15.0
npm config set prefix xxx路径 # 此时全局npm包安装的路径就是 xxx路径 了
# 比如安装在 ~/.npm-global/lib/node_modules 下，而二进制执行文件在~/.npm-global/bin路径，因此就需要将这个路径配置到环境变量里
# 而系统找不到，是因为咱再终端里一般想直接 lerna xxx 调用，此时就需要配置环境变量，从而让电脑去找到这个安装包
# 而格式就是：export PATH=xxx路径:$PATH，后面的:$PATH是拼接其他地方的环境变量。
# 查看配置的.zshrc里有下面一行代码，和上面第一个环境变量相同
export PATH=~/.npm-global/bin:$PATH
# 通过export就可以将变量输出到 所有的环境变量里，可以 echo $PATH 查看是否配置成功
...
- 读取顺序：/etc/profile  -》 /etc/paths -》 ~/.bash_profile 或 ~/.bashrc -》 /etc/bashrc
- 其实后面的覆盖前面的，如果指定对应的环境变量文件，则优先级会更高
- $path 与 $PATH 都是变量，前者打印的单个的值，而后者是拼接好的值
- $HOME 其实就是个人的家目录，比如：/Users/yq

```bash
# 在 ~/.zshrc 文件里，
# $PATH 是个变量，这里指所有的环境变量都在 $HOME/.npm-global/bin 之后
export PATH="$HOME/.npm-global/bin:$PATH"
# 而 /opt/homebrew/bin 则表示，在所有环境变量之后拼接
...
- 可以在包目录使用npm link，package.json里必须有name和main字段，name就是npm link要使用的字段，main则是模块加载使用字段，使用时只需要npm link xxx_name即可
- 如果想取消link，则需要在对应目录执行：npm unlink
...
5. you must verify your email before publishing a new package
...
export PATH=~/.npm-global/bin:$PATH
#第四步：update profile。使其生效(注意source命令只针对当前终端有效) source命令一般针对的是类似 npmrc这种配置文件
source ~/.profile

# Overall, the source xxx command is used to run a script or configuration file and update the current shell environment with the settings defined in the script or file. 
```
...
基于 IP 或域名的主机名分离技术是一种将多个域名或主机名映射到同一个 IP 地址的技术。其原理是通过 Web 服务器软件来实现，服务器软件可以通过 DNS 查找获取域名对应的 IP 地址，当客户端通过浏览器访问某个域名时，DNS 会将域名解析为对应的 IP 地址，然后将请求发送给对应的服务器，服务器根据请求的域名来匹配对应的虚拟主机配置，以便为客户端提供正确的内容。虚拟主机配置可以根据域名或主机名来设置，将多个域名或主机名映射到同一个 IP 地址，并为其提供不同的内容，从而实现在同一个物理服务器上运行多个虚拟主机。这种技术可以极大地提高服务器的利用率和灵活性。
