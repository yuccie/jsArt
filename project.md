...
总的来说，Flutter 应用程序在运行时会经过多个层次的处理和渲染，从 Dart 代码到 Native 代码，再到引擎渲染和交互，最终展现在用户面前。
## Dart 概览
...
- 函数一般来说，只应该做一件事，就是返回一个值。
  - 如果你有多个操作，每个操作应该写成一个单独的函数。而且，**数据的状态应该与操作方法**分离。根据这种理念，React 的函数组件只应该做一件事情：返回组件的 HTML 代码，而没有其他的功能。
  - 这个函数只做一件事，就是根据输入的参数，返回组件的 HTML 代码。这种只进行单纯的数据计算（换算）的函数，在函数式编程里面称为 "纯函数"（pure function）。
- 如果纯函数只能进行数据计算，那些涉及计算的操作（比如生成日志、储存数据、改变应用状态等等）应该写在哪里呢？
  - 函数式编程将那些跟数据计算无关的操作，都称为 "副效应" （side effect） 。如果函数内部直接包含产生副效应的操作，就不再是纯函数了，我们称之为不纯的函数。
  - 纯函数内部只有**通过间接的手段（即通过其他函数调用），才能包含副效应**。
...
Lerna 是一个用于管理多个包存储在单个 git 仓库中的工具，而 Monorepo 是一种将多个相关项目存储在一个单独的代码库中的开发方式。Lerna 可以用来优化在 Monorepo 中管理多个包的工作流程，使得开发者可以更方便地维护和共享代码。使用 Lerna 管理 Monorepo 可以方便开发者在多个项目中共享代码，同时也可以更轻松地管理依赖关系，使得项目变得更易于维护。

Monorepo是指将多个项目存储在同一个代码仓库中，这些项目之间可以共享代码、工具、构建配置等资源，实现高效的开发和部署。

通常情况下，一个团队会有多个项目，每个项目都有自己的代码仓库。但是，当这些项目之间有共享的代码或依赖关系时，维护多个独立的代码仓库会带来许多问题，如代码重复、依赖管理、构建和部署等。

通过采用Monorepo的方式，可以将这些项目整合到一个代码仓库中，统一管理和维护。这样可以避免重复代码、提高代码重用率，同时也能够更方便地管理依赖、升级版本、构建和部署等，从而提高开发效率和代码质量。

Yarn Workspaces和Lerna都是用来管理Monorepo（多包存储库）的工具，但它们在实现上有些不同。具体来说，它们的区别如下：

- 实现方式不同：Yarn Workspaces 是内置在Yarn中的特性，而Lerna则是一个单独的CLI工具，需要单独安装。
- 功能不同：Yarn Workspaces 主要用于管理依赖关系，它将多个包（package）视为一个整体来进行安装和更新。Lerna则不仅提供了类似的依赖管理功能，还包含了诸如发布、测试和构建等功能。
- 配置方式不同：Yarn Workspaces 需要在package.json中配置"workspaces"属性来指定包含的所有包。而Lerna需要在lerna.json或者通过命令行选项来配置。
- 应用场景不同：Yarn Workspaces 更适合于纯粹的依赖管理，而Lerna则更适合于大型的Monorepo项目，其中需要处理多个相互依赖的包，以及进行自动化的发布、测试和构建等操作。

总之，Yarn Workspaces和Lerna都是很好的管理Monorepo的工具，具体使用哪个取决于项目的具体需求。

#### lerna bootstrap

在一个多包的工作区中，不同的包之间很可能存在相互依赖的情况，而这些依赖的管理可能会比较复杂。为了让多个包之间的依赖更加清晰明了，可以使用 lerna bootstrap 命令将这些包之间的依赖关系链接在一起。

具体来说，lerna bootstrap 命令会在当前工作区中的所有包中安装依赖，然后通过符号链接的方式将包之间的依赖关系建立起来，使得不同包之间的依赖可以直接使用，而不需要通过 npm 等包管理工具进行安装。这样可以大大简化包之间的依赖关系管理，提高项目的开发效率。

#### lerna publish 的工作原理如下：

1. 确定哪些包需要发布：lerna changed 命令会检查每个包的变更记录，并输出需要被发布的包。
2. 更新每个包的版本号：根据命令行参数、Git commit message 或其他配置文件，自动更新每个包的版本号，并更新每个包的 package.json 文件。
3. 发布每个包：对于每个需要发布的包，Lerna 会在 npm registry 中发布一个新版本。
4. 创建 Git tag：每个发布版本会自动在 Git 中打一个 tag，tag 名称为 v<package-version>。

**注意，**如果你的 packages 使用了 Git dependencies，则它们的版本号也会自动更新并被提交到 Git 仓库中。Git dependencies 指的
是在 package.json 文件中声明的依赖为一个 Git 仓库地址，而不是指向一个已经发布到 npm registry 的包。
